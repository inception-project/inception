...
public class AnnotationServiceImpl implements AnnotationService {
	...
	@Override
	@Transactional
	public void createTag(Tag aTag, User aUser) throws IOException {
		entityManager.persist(aTag);

		RepositoryServiceDbData.createLog(aTag.getTagSet().getProject(),
				aUser.getUsername()).info(
				" Added tag [" + aTag.getName() + "] with ID [" + aTag.getId()
						+ "] to TagSet [" + aTag.getTagSet().getName() + "]");
		RepositoryServiceDbData.createLog(aTag.getTagSet().getProject(),
				aUser.getUsername()).removeAllAppenders();
	}

	@Override
	@Transactional
	public void createTagSet(TagSet aTagSet, User aUser) throws IOException {
		...
		RepositoryServiceDbData.createLog(aTagSet.getProject(),
				aUser.getUsername()).info(
				" Added tagset  [" + aTagSet.getName() + "] with ID ["
						+ aTagSet.getId() + "]");
		RepositoryServiceDbData.createLog(aTagSet.getProject(),
				aUser.getUsername()).removeAllAppenders();
	}
	...
	public void createLayer(AnnotationLayer aType, User aUser)
		...
		RepositoryServiceDbData.createLog(aType.getProject(),
				aUser.getUsername()).info(
				" Added tagset  [" + aType.getName() + "] with ID ["
						+ aType.getId() + "]");
		RepositoryServiceDbData.createLog(aType.getProject(),
				aUser.getUsername()).removeAllAppenders();
	}
	...
	public Tag getTag(String aTagName, TagSet aTagSet) {
		return entityManager
				.createQuery(
						"FROM Tag WHERE name = :name AND" + " tagSet =:tagSet",
						Tag.class).setParameter("name", aTagName)
	...
	public boolean existsTagSet(String aName, Project aProject) {
		try {
			entityManager
					.createQuery(
							"FROM TagSet WHERE name = :name AND project = :project",
	...
	public boolean existsTagSet(Project aProject) {
		try {
			entityManager
					.createQuery("FROM TagSet WHERE  project = :project",
							TagSet.class).setParameter("project", aProject)
					.getSingleResult();
			return true;
		} catch (NoResultException e) {
			return false;
	...
	public boolean existsFeature(String aName, AnnotationLayer aLayer) {
		...
		try {
			entityManager
					.createQuery(
							"FROM AnnotationFeature WHERE name = :name AND layer = :layer",
							AnnotationFeature.class)
					.setParameter("name", aName).setParameter("layer", aLayer)
					.getSingleResult();
			return true;
		} catch (NoResultException e) {
			return false;
	...
	public TagSet getTagSet(long aId) {
		return entityManager
				.createQuery("FROM TagSet WHERE id = :id", TagSet.class)
	...
	public AnnotationLayer getLayer(long aId) {
		return entityManager
				.createQuery("FROM AnnotationLayer WHERE id = :id",
						AnnotationLayer.class).setParameter("id", aId)
				.getSingleResult();
	}
	...
	public AnnotationFeature getFeature(long aId) {
		return entityManager
				.createQuery("From AnnotationFeature where id = :id",
						AnnotationFeature.class).setParameter("id", aId)
				.getSingleResult();
	}
	...
	public boolean existsType(String aName, String aType) {
		try {
			entityManager
					.createQuery(
							"From AnnotationLayer where name = :name AND type = :type",
							AnnotationLayer.class).setParameter("name", aName)
	...
	private AnnotationFeature initializeType(String aName, String aUiName,
			String aDescription, String aType, String aTagSetName,
			String aLanguage, String[] aTags, String[] aTagDescription,
	...
	public void initializeTypesForProject(Project aProject, User aUser,
			String[] aPostags, String[] aPosTagDescriptions, String[] aDepTags,
			String[] aDepTagDescriptions, String[] aNeTags,
			String[] aNeTagDescriptions, String[] aCorefTypeTags,
			String[] aCorefRelTags) throws IOException {
	...
	@Override
	public void createCorefLayer(Project aProject, User aUser,
			String[] aCorefTypeTags, String[] aCorefRelTags) throws IOException {
		// Coref Layer
		AnnotationFeature corefTypeFeature = initializeType("referenceType",
				"referenceType", "coreference type annotation",
				"de.tudarmstadt.ukp.dkpro.core.api.coref.type.Coreference",
				"BART", "de", aCorefTypeTags.length > 0 ? aCorefTypeTags
						: new String[] { "nam" },
				aCorefTypeTags.length > 0 ? aCorefTypeTags
		...
		AnnotationFeature corefRelFeature = initializeType("referenceRelation",
				...
				"de.tudarmstadt.ukp.dkpro.core.api.coref.type.Coreference",
				"TuebaDZ", "de", aCorefRelTags.length > 0 ? aCorefRelTags
						: new String[] { "anaphoric" },
				aCorefRelTags.length > 0 ? aCorefRelTags
	...
	public void createNeLayer(Project aProject, User aUser, String[] aNeTags,
			String[] aNeTagDescriptions) throws IOException {
		// NE layer

		String[] neTags = aNeTags.length > 0 ? aNeTags : new String[] { "PER",
				"PERderiv", "PERpart", "LOC", "LOCderiv", "LOCpart", "ORG",
				"ORGderiv", "ORGpart", "OTH", "OTHderiv", "OTHpart" };
		String[] neTagDescriptions = aNeTagDescriptions.length == neTags.length ? aNeTagDescriptions
				: new String[] {
						"Person",
						"Person derivative",
						"Hyphenated part  is person",
						"Location derivatives",
						"Location derivative",
						"Hyphenated part  is location",
						"Organization",
	...
	public void createDepLayer(Project aProject, User aUser, String[] aDepTags,
			String[] aDepTagDescriptions) throws IOException {
		// Dependency Layer
		String[] depTags = aDepTags.length > 0 ? aDepTags : new String[] {
				"ADV", "APP", "ATTR", "AUX", "AVZ", "CJ", "DET", "ETH", "EXPL",
				"GMOD", "GRAD", "KOM", "KON", "KONJ", "NEB", "OBJA", "OBJA2",
				"OBJA3", "OBJC", "OBJC2", "OBJC3", "OBJD", "OBJD2", "OBJD3",
				"OBJG", "OBJG2", "OBJG3", "OBJI", "OBJI2", "OBJI3", "OBJP",
				"OBJP2", "OBJP3", "PAR", "PART", "PN", "PP", "PRED", "-PUNCT-",
				"REL", "ROOT", "S", "SUBJ", "SUBJ2", "SUBJ3", "SUBJC",
				"SUBJC2", "SUBJC3", "SUBJI", "SUBJI2", "CP", "PD", "RE", "CD",
				"DA", "SVP", "OP", "MO", "JU", "CVC", "NG", "SB", "SBP", "AG",
				"PM", "OCRC", "OG", "SUBJI3", "VOK", "ZEIT", "$", "--", "OC",
				"OA", "MNR", "NK", "RC", "EP", "CC", "CM", "UC", "AC", "PNC" };
	...
	public void createPOSLayer(Project aProject, User aUser, String[] aPostags,
			String[] aPosTagDescriptions) throws IOException {
		// POS layer
		String[] posTags = aPostags.length > 0 ? aPostags : new String[] {
				"$(", "$,", "$.", "ADJA", "ADJD", "ADV", "APPO", "APPR",
				"APPRART", "APZR", "ART", "CARD", "FM", "ITJ", "KOKOM", "KON",
				"KOUI", "KOUS", "NE", "NN", "PAV", "PDAT", "PDS", "PIAT",
				"PIDAT", "PIS", "PPER", "PPOSAT", "PPOSS", "PRELAT", "PRELS",
				"PRF", "PROAV", "PTKA", "PTKANT", "PTKNEG", "PTKVZ", "PTKZU",
	...
	private AnnotationLayer setLayer(String aName, String aFeatureName,
			String aUiName, String aType, Project aProject) {
	...
	private AnnotationFeature setFeature(String aName, String aUiname,
			Project aProject, AnnotationLayer aLayer, String aType) {
		AnnotationFeature feature = new AnnotationFeature();
		feature.setName(aName);
	...
	public List<AnnotationLayer> listAnnotationLayer(Project aProject) {
		return entityManager
				.createQuery(
						"FROM AnnotationLayer WHERE project =:project ORDER BY uiName",
	...
	public List<AnnotationFeature> listAnnotationFeature(AnnotationLayer aLayer) {
		...
		return entityManager
				.createQuery(
						"FROM AnnotationFeature  WHERE layer =:layer ORDER BY uiName",
						AnnotationFeature.class).setParameter("layer", aLayer)
				.getResultList();
	}
	...
	public List<Tag> listTags() {
		return entityManager.createQuery("From Tag ORDER BY name", Tag.class)
				.getResultList();
	}
	...
	public List<Tag> listTags(TagSet aTagSet) {
		List<Tag> tags = entityManager
				.createQuery(
						"FROM Tag WHERE tagSet = :tagSet ORDER BY name ASC",
						Tag.class).setParameter("tagSet", aTagSet)
				.getResultList();
	...
	public List<TagSet> listTagSets() {
		return entityManager.createQuery("FROM TagSet ORDER BY name ASC",
				TagSet.class).getResultList();
	}
	...
	public List<TagSet> listTagSets(Project aProject) {
		return entityManager
				.createQuery(
						"FROM TagSet where project = :project ORDER BY name ASC",
						TagSet.class).setParameter("project", aProject)
				.getResultList();
	}
