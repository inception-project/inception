...
Now we have a dedicated Apache Tomcat instance for WebAnno installed at `/opt/webanno/`. Put the `webanno.war` file in the `/opt/webanno/webapps` folder. If you get the war file in different name, such as `webanno-webapp-3.0.war`, *PLEASE RE-NAME IT TO* `webanno.war`
