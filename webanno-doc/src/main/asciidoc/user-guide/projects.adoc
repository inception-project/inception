...
A *condition* is a way of defining whether a particular situation in webanno is based on annotation layers and features in it.
	<path> = <value>
A *condition set* consists of one or more conditions. All conditions are logically ANDed during evaluation. 
	<condition> & <condition>
In contrast to conditions, *restrictions* may contain a *flag* such as “(!)” at the end which marks the importance of the restriction. A typical restriction looks like 
	<path> = <value> (!)
A *restriction set* defines a set of restrictions which can be applied if a particular condition set is evaluated to true. A restriction set consists of several restrictions which are independant of each other.
<Restriction> | <Restriction>


Note: The important flag part is option. The value should always be in double quotes.
...
*Feature selectors* consist of a feature name.


*Navigation across layers* is possible via 
	@<Type>
Hereby all annotations of type <Type> at the same position as the current context annotation are found.

==Examples:

Dependency relation
	POS type of governor/dependent constraints dependency type
	Example: 
Dependency {
  type = governor.pos.posValue = “VB” & dependent.pos.posValue = “PRP”  ->
    depRel = “nsubj”; 
}
Semantic role labelling
	Frame type constrains types of arguments
	Example: frame = “Assistance” -> arguments.role = “Manner” | arguments.role =  ”Helper” | arguments.role = “Beneficiary”
	Constraint on semantic predicate based on lemma
	If a lemma exists with the same offsets as the current annotation (semantic predicate), then the frame feature of the semantic predicate is constrained to “Assistance”.
	Example importing a fully qualified name
import de.tudarmstadt.ukp.dkpro.types.Lemma as Lemma
@Lemma.value = “assist” -> frame = “Assistance”
