...
In the last frame, *Behaviours*, other properties of the annotation may be selected. 
The field *Lock to token offsets*, gives the possibility to bind the annotation to a token.
The field *Allow stacking* gives the possibility to make several annotations over one range.
The field *Allow crossing sentence boundary* gives the possibility to annotate a range covering more than one sentence. 
The field *Readonly* controls whether annotations in this layer can be created or edited.
