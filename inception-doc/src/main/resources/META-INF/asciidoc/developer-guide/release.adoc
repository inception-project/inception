...
. Check that the release branch depends on the right webanno release or update it in the most high-level `pom.xml` (properties -> `webanno.version`).
...
Check out the release tag e.g. via `git checkout tags/inception-app-0.8.3`. Make sure that in the master pom, the version is set to
...
After successfully publishing to docker hub, the docker hub repository will show your released version as the latest version.
