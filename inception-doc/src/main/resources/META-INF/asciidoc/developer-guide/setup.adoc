...
NOTE: Should the steps mentioned above not have been sufficient, close all the {product-name} projects
      ...
      If the Maven project update cannot be completed due to missing .jars, execute a Maven install via right click *Run as ->
      Maven build...*, enter the goal `install` and check *Skip Tests*. Alternatively, use the command `mvn install -DskipTests`.
