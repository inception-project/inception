...
NOTE: The script contains placeholder for usernames and passwords used by the {product-name} container to
       connect to the database container, namely `DBUSER` and `DBPASSWORD`. You either need to replace
       them in the compose file or set them, e.g. via
