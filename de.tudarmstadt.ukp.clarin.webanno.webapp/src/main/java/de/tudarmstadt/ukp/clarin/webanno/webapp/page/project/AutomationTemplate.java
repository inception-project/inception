...
/**
 * A Panel used to add automation template used by MIRA for an automation {@link Project}
 *
    ...
    extends Panel
{
    ...
    public AutomationTemplate(String id, Model<Project> aProjectModel)
    {
        super(id);
        this.selectedProjectModel = aProjectModel;
        add(fileUpload = new FileUploadField("content", new Model()));

        add(new Button("importTemplate", new ResourceModel("label"))
        {
            ...
            public void onSubmit()
            {
                ...
                for (FileUpload templateFile : uploadedFiles) {

                    try {
                        File tempFile = templateFile.writeToTempFile();
                        String fileName = templateFile.getClientFileName();
                        String username = SecurityContextHolder.getContext().getAuthentication()
                                .getName();
                        repository.createTemplate(project, tempFile, fileName, username);
                    }
                    catch (IOException e) {
...

            {
                setChoices(new LoadableDetachableModel<List<String>>()
                {
                    ...
                    protected List<String> load()
                    {
                        ...
                        documents.clear();
                        if (project.getId() != 0) {
                            documents.addAll(repository.listTemplates(project));
                        }
                        return documents;
                    }
                });
            ...
            public void onSubmit()
            {
                ...
                for (String document : selectedDocuments) {
                    try {
                        String username = SecurityContextHolder.getContext().getAuthentication()
                                .getName();
                        repository.removeTemplate(project, document, username);
                    }
                    catch (IOException e) {
