...
public class BratCuratorUtility {
	...
	public static void mergeSpan(IRequestParameters aRequest,
		...
		Project project = aCurationUserSegment.getBratAnnotatorModel()
				.getProject();
		...
		if (annotationSelection != null) {
			...
			try {
				createSpan(aRequest,
						aCurationUserSegment.getBratAnnotatorModel(), aJcas,
						clickedAnnotationDocument, address, aRepository,
						aAnnotationService);
	...
	public static void mergeArc(IRequestParameters aRequest,
		...
		if (annotationSelectionOrigin != null
			...
			try {
				if (aCurationUserSegment.getBratAnnotatorModel().getMode()
						.equals(Mode.CORRECTION)) {
					clickedJCas = repository
							.getCorrectionDocumentContent(aCurationUserSegment
									.getBratAnnotatorModel().getDocument());
				} else {
					clickedJCas = repository
			...
			try {
				controller.addArcToCas(
						aCurationUserSegment.getBratAnnotatorModel(), arcType,
						0, 0, addressOrigin, addressTarget, aJcas);
	...
	public static void createSpan(IRequestParameters aRequest,
		...
		if (aBratAnnotatorModel.getMode().equals(Mode.CORRECTION)) {
			clickedJCas = aRepository
					.getCorrectionDocumentContent(aBratAnnotatorModel
							.getDocument());
		} else {
			clickedJCas = aRepository
		...
		// sentence annotation
		// views
		// spanType = BratAjaxCasUtil.getAnnotationType(fsClicked.getType()) +
		// spanType;

		BratAjaxCasController controller = new BratAjaxCasController(
				aRepository, aAnnotationService);

		controller.addSpanToCas(aMergeJCas, fsClicked.getBegin(),
				fsClicked.getEnd(), spanType, 0, 0);
	...
	/**
	 * Set different attributes for {@link BratAnnotatorModel} that will be used
	 * for the {@link CurationSegmentForSourceDocument}
	 * 
	 * @throws IOException
	...
	public static BratAnnotatorModel setBratAnnotatorModel(
			SourceDocument aSourceDocument, RepositoryService aRepository,
			CurationSegmentForSourceDocument aCurationSegment,
			AnnotationService aAnnotationService) throws BeansException,
			FileNotFoundException, IOException {
		User userLoggedIn = aRepository.getUser(SecurityContextHolder
		...
		bratAnnotatorModel.setFirstSentenceAddress(aCurationSegment
				.getSentenceAddress().get(CURATION_USER));
		bratAnnotatorModel.setLastSentenceAddress(aCurationSegment
				.getSentenceAddress().get(CURATION_USER));
		bratAnnotatorModel.setSentenceAddress(aCurationSegment
				.getSentenceAddress().get(CURATION_USER));
		bratAnnotatorModel.setMode(Mode.CURATION);
		AnnotationPreference preference = new AnnotationPreference();
		ApplicationUtils.setAnnotationPreference(preference,
	...
	public static void fillLookupVariables(
		...
		for (AnnotationOption annotationOption : aAnnotationOptions) {
			for (AnnotationSelection annotationSelection : annotationOption
				...
				for (String username : annotationSelection
						.getAddressByUsername().keySet()) {
					if ((!username.equals(CURATION_USER) && bratAnnotatorModel
							.getMode().equals(Mode.CURATION))
							|| (username.equals(CURATION_USER) && bratAnnotatorModel
									.getMode().equals(Mode.CORRECTION))) {
						Integer address = annotationSelection
						...
						aAnnotationSelectionByUsernameAndAddress.get(username)
								.put(address, annotationSelection);
					}
				}
			}
		}
	...
	public static void populateCurationSentences(
		...
		for (String username : usernamesSorted) {
			if ((!username.equals(CURATION_USER) && aBratAnnotatorModel
					.getMode().equals(Mode.CURATION))
					|| (username.equals(CURATION_USER) && aBratAnnotatorModel
							.getMode().equals(Mode.CORRECTION))) {
				...
				for (AnnotationOption annotationOption : aAnnotationOptions) {
					for (AnnotationSelection annotationSelection : annotationOption
							.getAnnotationSelections()) {
						if (annotationSelection.getAddressByUsername()
								.containsKey(username)) {
							Integer address = annotationSelection
									.getAddressByUsername().get(username);
							annotationSelectionByAddress.put(address,
									annotationSelection);
						}
					}
				}
				...
				BratAjaxCasController.addBratResponses(response,
						aBratAnnotatorModel, 0, jCas, true);

				CurationUserSegmentForAnnotationDocument curationUserSegment2 = new CurationUserSegmentForAnnotationDocument();
				curationUserSegment2.setCollectionData(getStringCollectionData(
						response, jCas, annotationSelectionByAddress, username,
						numUsers, aJsonConverter, aBratAnnotatorModel,
						aAnnotationOptions));
				curationUserSegment2
						.setDocumentResponse(getStringDocumentResponse(
								response, aJsonConverter));
	...
	private static String getStringDocumentResponse(
		...
		JsonGenerator jsonGenerator = aJsonConverter.getObjectMapper()
	...
	private static String getStringCollectionData(GetDocumentResponse response,
			JCas jCas,
			Map<Integer, AnnotationSelection> annotationSelectionByAddress,
			String username, int numUsers,
			MappingJacksonHttpMessageConverter aJsonConverter,
			BratAnnotatorModel aBratAnnotatorModel,
			List<AnnotationOption> aAnnotationOptions) throws IOException {
		Map<String, Map<String, Object>> entityTypes = new HashMap<String, Map<String, Object>>();
		if (aBratAnnotatorModel.getMode().equals(Mode.CORRECTION)) {
			getCorrectionEntities(response, annotationSelectionByAddress,
					aAnnotationOptions, numUsers, entityTypes);
		} else {
			getCurationEntities(response, annotationSelectionByAddress,
	...
	private static void getCurationEntities(GetDocumentResponse response,
			Map<Integer, AnnotationSelection> annotationSelectionByAddress,
			int numUsers, Map<String, Map<String, Object>> entityTypes) {
		...
		for (Entity entity : response.getEntities()) {
			...
			targetAnnotations.put(entity.getId(), entity.getType() + "_("
					+ newState.name() + ")");
		}
		for (Entity entity : response.getEntities()) {
			...
			if (newState != null) {
				...
				for (Relation relation : response.getRelations()) {
					...
					if (argument.getToken() == entity.getId()) {// has outgoing
						...
						List<RelationType> relations = getCurationRelations(
								...
								numUsers,
								relation,
								targetAnnotations.get(relation.getArguments()
										.get(1).getToken()));
						Map<String, Object> enityTypeWithArcs = BratCuratorUtility
								.getEntity(type, label, newState);
						if (entityTypes.get(type) == null
								|| (entityTypes.get(type) != null && entityTypes
										.get(type).get("arcs") == null)) {
				...
				if (!hasArc) {
					if (entityTypes.get(type) == null
							|| (entityTypes.get(type) != null && entityTypes
									.get(type).get("arcs") == null)) {
						entityTypes.put(type, BratCuratorUtility.getEntity(
								type, label, newState));
					}
				}
			}
	...
	private static AnnotationState getState(int numUsers,
		...
		if (annotationSelection == null) {
		...
		} else if (annotationSelection.getAddressByUsername().containsKey(
				CURATION_USER)) {
			...
			for (AnnotationSelection otherAnnotationSelection : annotationSelection
				...
				if (otherAnnotationSelection.getAddressByUsername()
						.containsKey(CURATION_USER)) {
					doNotUse = true;
					break;
				}
			}
	...
	private static List<RelationType> getCurationRelations(
			GetDocumentResponse response,
			Map<Integer, AnnotationSelection> annotationSelectionByAddress,
			int numUsers, Relation relation, String arcTarget) {
		...
		if (newState != null) {
			...
			String label = relation.getType().replace(
					AnnotationTypeConstant.POS_PREFIX, "").replace(
							AnnotationTypeConstant.COREFERENCE_PREFIX, "");
			relation.setType(type);
			return getRelation(type, label, newState,
					Arrays.asList(new String[] { arcTarget }));
		}
	...
	public static List<RelationType> getRelation(String type, String label,
		...
		RelationType arc = new RelationType(annotationState.getColorCode(),
				"triangle,5", Arrays.asList(label), type, arcTargets, "");
		arcs.add(arc);
		return arcs;
	}
		...
		for (Entity entity : response.getEntities()) {
			...
			for (AnnotationOption annotationOption2 : aAnnotationOptions) {
				for (AnnotationSelection annotationSelection2 : annotationOption2
						.getAnnotationSelections()) {
					if (annotationSelection2.getAddressByUsername()
							.containsKey(CURATION_USER)
							&& annotationSelection2.getAddressByUsername().get(
									CURATION_USER) == address) {
			...
			if (newState != null) {
				...
				String label = entity.getType();
				entity.setType(type);
				entityTypes.put(type,
						BratCuratorUtility.getEntity(type, label, newState));
			}
	...
	public static void updatePanel(
			...
			MappingJacksonHttpMessageConverter aJsonConverter)
			throws UIMAException, ClassNotFoundException, IOException {
		SourceDocument sourceDocument = aCurationContainer
				.getBratAnnotatorModel().getDocument();
		Project project = aCurationContainer.getBratAnnotatorModel()
				.getProject();
		List<AnnotationDocument> annotationDocuments = aRepository
				.listAnnotationDocument(project, sourceDocument);
		...
		if (aCurationContainer.getBratAnnotatorModel().getMode()
				.equals(Mode.CORRECTION)) {
			annotatorCas = aRepository
			...
			jCases.put(user.getUsername(), aRepository
					.getAnnotationDocumentContent(annotationDocument));
			...
			BratCuratorUtility.getCases(jCases, annotationDocuments,
					aRepository, aAnnotationSelectionByUsernameAndAddress);
		}
		...
		if (!aCurationContainer.getBratAnnotatorModel().getMode()
				.equals(Mode.CORRECTION)) {
			bratAnnotatorModel = BratCuratorUtility.setBratAnnotatorModel(
					sourceDocument, aRepository, aCurationSegment,
					aAnnotationService);
		} else {
		...
		BratCuratorUtility.populateCurationSentences(jCases, sentences,
				bratAnnotatorModel, annotationOptions,
