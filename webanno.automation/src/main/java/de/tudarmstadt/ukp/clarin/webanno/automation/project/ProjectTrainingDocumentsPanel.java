...
public class ProjectTrainingDocumentsPanel extends Panel {
	...
	public ProjectTrainingDocumentsPanel(String id,
			Model<Project> aProjectModel, final boolean aFreeDocument,
		...
		try {
			if (aFreeDocument) {
				...
				readableFormats = new ArrayList<String>(
						repository.getReadableFormatLabels());
		...
		add(readableFormatsChoice = new DropDownChoice<String>(
				"readableFormats", new Model(selectedFormat), readableFormats) {
				...
				for (FileUpload documentToUpload : uploadedFiles) {
					...
					try {
						...
						if (aFreeDocument) {
							...
							String reader = repository
									.getReadableFormatId(readableFormatsChoice
											.getModelObject());
		...
		add(new ListMultipleChoice<String>("documents", new Model(
			...
			public void onSubmit() {
				...
				for (String document : selectedDocuments) {
					try {
						...
						User user = repository.getUser(username);
						repository
								.removeSourceDocument(repository
										.getSourceDocument(project, document),
										user);
					} catch (IOException e) {
