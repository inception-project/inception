...
For the actual evaluation, we collect the true label and the predicted majority label in an 
`AnnotatedTokenPair` for each true label. A stream of these instances is then passed to the result 
of the evaluation - a `EvaluationResult` object. This object provides access to calculations for 
token-based accuracy, macro-averaged precision, recall and F1-score. This F1-score is later used for 
