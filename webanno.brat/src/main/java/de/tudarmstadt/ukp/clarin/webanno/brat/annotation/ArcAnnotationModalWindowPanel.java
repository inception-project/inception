...
public class ArcAnnotationModalWindowPanel extends Panel {
	...
	private class AnnotationDialogForm extends Form<SelectionModel> {
		...
		public AnnotationDialogForm(String id, final ModalWindow aModalWindow) {
			...
			for (AnnotationFeature feature : annotationService
				...
				} else if (bratAnnotatorModel.getRememberedArcFeatures().get(
						feature) != null) {
				...
				protected void populateItem(final Item<FeatureValue> item) {
					...
					item.add(new ComboBox<Tag>(
							...
							(item.getModelObject().feature.getTagset() != null && item
									...
									.listTags(item.getModelObject().feature
											.getTagset())
									: new ArrayList<Tag>(),
			...
			add(new AjaxButton("annotate") {
				...
				public void onSubmit(AjaxRequestTarget aTarget, Form<?> aForm) {
					...
					try {
						...
						for (IModel<String> model : tagModels) {
							...
							if (adapter instanceof ArcAdapter) {
								...
								((ChainAdapter) adapter).add(
										...
										originFs.getBegin(), targetFs.getEnd(),
										originFs, targetFs, feature);
							}
							if (tag.equals("")) {
						...
						bratAnnotatorModel
								.setRememberedArcFeatures(selectedFeatureValues);
					...
					try {
						...
						if (adapter instanceof ArcAdapter) {
							for (IModel<String> model : tagModels) {
								AnnotationFeature feature = featureModels.get(
										tagModels.indexOf(model)).getObject().feature;
								((ArcAdapter) adapter).add(model.getObject(),
										targetFs, originFs, jCas,
										bratAnnotatorModel, feature);
							}
						} else {
	...
	public ArcAnnotationModalWindowPanel(String aId,
		...
		if (id.contains("_")) {
			...
			this.selectedArcType = this.selectedArcType.substring(aType
					.indexOf("_") + 1);
		} else {
