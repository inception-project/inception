...
public class SpanAnnotationModalWindowPage extends WebPage {
	...
	private class AnnotationDialogForm extends Form<SelectionModel> {
		...
		public AnnotationDialogForm(String id, final ModalWindow aModalWindow) {
			...
			addSpanLayers: for (AnnotationLayer layer : bratAnnotatorModel
					.getAnnotationLayers()) {
				if (!layer.isEnabled()
			...
			add(new DropDownChoice<AnnotationLayer>("layers", layersModel,
				...
				protected void onSelectionChanged(AnnotationLayer aNewSelection) {
					...
					for (AnnotationFeature feature : annotationService
						...
						if (aNewSelection.getType().equals(
								WebAnnoConst.CHAIN_TYPE)
								&& feature
										.getName()
										.equals(WebAnnoConst.COREFERENCE_RELATION_FEATURE)) {
							continue;
						}
			...
			for (AnnotationFeature feature : annotationService
				...
				} else if (bratAnnotatorModel.getRememberedSpanFeatures().get(
						feature) != null) {
					tagModel.setObject(bratAnnotatorModel
							.getRememberedSpanFeatures().get(feature));
				}
				tagModels.add(tagModel);
			}

			add(featureValues = new RefreshingView<FeatureValue>(
					"featureValues") {
				...
				protected void populateItem(final Item<FeatureValue> item) {
					item.add(new Label(
							...
							item.getModelObject().feature.getTagset() == null ? item
									...
									: item.getModelObject().feature.getTagset()
											.getName()));
					List<Tag> tags = new ArrayList<Tag>();
					if (item.getModelObject().feature.getTagset() != null
							&& item.getModelObject().feature.getTagset() != null) {
						tags.addAll(annotationService.listTags(item
								.getModelObject().feature.getTagset()));
					}
					item.add(new ComboBox<Tag>("tag", tagModels.get(item
							.getIndex()), tags, new ComboBoxRenderer<Tag>(
							"name", "name")));
				}
			...
			add(new AjaxButton("annotate") {
				...
				protected void onSubmit(AjaxRequestTarget aTarget, Form<?> form) {
					...
					try {
						...
						for (IModel<String> model : tagModels) {
							...
							if (feature.getTagset() == null) {
								...
								annotationService.createTag(selectedTag,
									...
									model.getObject(), feature.getTagset())) {
								aTarget.add(feedbackPanel);
							...
							if (adapter instanceof SpanAdapter) {
								((SpanAdapter) adapter)
										.setLockToTokenOffsets(selectedLayer
												.isLockToTokenOffset());
								((SpanAdapter) adapter)
										.setAllowStacking(selectedLayer
												.isAllowSTacking());
								((SpanAdapter) adapter)
										.setAllowMultipleToken(selectedLayer
												.isMultipleTokens());
								((SpanAdapter) adapter)
										.setCrossMultipleSentence(selectedLayer
												.isCrossSentence());
								((SpanAdapter) adapter).add(jCas, beginOffset,
										endOffset, feature,
										selectedTag.getName());

							} else {

								((ChainAdapter) adapter).add(
										selectedTag.getName(), jCas,
						...
						bratAnnotatorModel
								.setRememberedSpanLayer(selectedLayer);
						bratAnnotatorModel
								.setRememberedSpanFeatures(selectedFeatureValues);
				...
				public void onSubmit(AjaxRequestTarget aTarget, Form<?> aForm) {
					try {
						...
						for (AnnotationLayer layer : annotationService
								.listAnnotationLayer(bratAnnotatorModel
										.getProject())) {
						...
						if (adapter instanceof ChainAdapter) {
							...
							TypeAdapter chainAdapter = new ChainAdapter(
									selectedLayer.getId(),
									selectedLayer.getName() + "Chain",
									selectedLayer.getName(), "first", "next");
							((ChainAdapter) adapter).updateCasBeforeDelete(
									jCas, selectedSpanId);
							((ChainAdapter) chainAdapter).delete(jCas,
									selectedSpanId);
							((ChainAdapter) adapter).removeInvalidChain(jCas
									.getCas());
						} else {
		...
		if (selectedLayer.getType().equals(WebAnnoConst.CHAIN_TYPE)) {
			...
			for (AnnotationFeature feature : annotationService
				...
				if (feature.isEnabled() || feature.isVisible()) {
					this.selectedFeatureValues.put(feature, st.nextToken()
							.trim());
				}
