...
public class RemoteApiController {
	 ...
	 * webanno.webapp/api/project '
	 *
	 * @param aName
	 *            the name of the project to create.
	 * @param aFileType
	 *            the type of the files contained in the ZIP. The possible file
	 *            types are configured in the formats.properties configuration
	 *            file of WebAnno.
	 * @param aFile
	...
	public @ResponseStatus(HttpStatus.NO_CONTENT) void createProject(@RequestParam("file") MultipartFile aFile,
		...
		// If the current filename does not start with "." and is in the root
		// folder of the ZIP,
		...
		for (Enumeration<?> zipEnumerate = zip.entries(); zipEnumerate.hasMoreElements();) {
			...
			// IF the current filename is
			// META-INF/webanno/source-meta-data.properties store it
			// as
			// project meta data
			...
			// If the current filename does not start with "." and is in the
			// root folder of the
	 ...
	 * 
	 * Test: Open your browser, paste following URL with appropriate values for
	 * username and password:
	 * 
	 * http://USERNAME:PASSWORD@localhost:8080/de.tudarmstadt.ukp.clarin.webanno
	 * .webapp/api/project/list
	 * 
	 * @return JSON string of project where user has access to and respective
	 *         roles in the project
	 * @throws Exception
	 */
	@RequestMapping(value = "/project/list", method = RequestMethod.GET)
	public @ResponseBody String listProject() throws Exception {
		...
		for (Project project : accessibleProjects) {
			...
			List<ProjectPermission> projectPermissions = projectRepository.listProjectPermisionLevel(user, project);
	...
	public @ResponseStatus(HttpStatus.NO_CONTENT) void deleteProject(@RequestParam("name") String aName) throws Exception {
		...
		boolean hasAccess = projectRepository.existsProjectPermissionLevel(user, project, PermissionLevel.ADMIN);
		...
		if (hasAccess) {
			...
			throw new PermissionDeniedDataAccessException("Not enough permission on [" + aName + "]",
	...
	/**
	 * Upload a document file into project where user has "ADMIN" role
	 * 
	 * Test: curl -v -F 'file=@test.txt' -F 'name=Test' -F 'filetype=txt'
	 * 'http://USERNAME:PASSWORD@localhost:8080/de.tudarmstadt.ukp.clarin.
	 * webanno.webapp/api/project/upload/documentfile'
	 * 
	 * 
	 * @param aFile
	 *            File
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/project/upload/documentfile", method = RequestMethod.POST, consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
	public @ResponseStatus(HttpStatus.NO_CONTENT) void uploadDocumentFile(@RequestParam("file") MultipartFile aFile,
			@RequestParam("name") String aName, @RequestParam("filetype") String aFileType) throws Exception {
		...
		if (hasAccess) {
			...
			boolean isDocumentPresent = projectRepository.existsSourceDocument(project, aFile.getOriginalFilename());
			if (!isDocumentPresent)
				uploadSourceDocumentFile(uploadDocumentFile, project, user, aFileType);
			else {
				throw new IOException("The source document with name [" + aFile.getOriginalFilename() + "] exists");
			}
		} else {
	...
	// @RequestMapping(value = "/project/upload/annotationfile", method =
	// RequestMethod.POST, consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
	// public @ResponseStatus(HttpStatus.NO_CONTENT) void
	// uploadAnnotationFile(@RequestParam("file") MultipartFile aFile,
	// @RequestParam("name") String aName, @RequestParam("filetype") String
	// aFileType) throws Exception {
	//
	// }
