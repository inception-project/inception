    ...
    extends AnnotationSidebar_ImplBase
{
    ...
    {
        return String.join("\n", "function getColorCache() {", "  var cacheHolder = window;",
                "  if (!cacheHolder.hasOwnProperty('colorCache')) {",
                "    console.debug('Initializing color cache');",
                "    cacheHolder.colorCache = {};", "  }", "  return cacheHolder.colorCache;", "}",
                ...
                "  try {", "    var color = getColorCache()[img.src];",
                "    if (color === undefined) {",
                "      var dominantColor = colorThief.getColor(img);",
                "      var r = dominantColor[0];", "      var b = dominantColor[1];",
                "      var g = dominantColor[2];", "      // http://alienryderflex.com/hsp.html",
                "      var hsp = Math.sqrt(0.299*r*r + 0.587*g*g + 0.114*b*b);",
                "      color = hsp > 127 ? 'black' : 'white';",
                "      getColorCache()[img.src] = color;", "    }",
                "    $(img).css('background-color', color);", "  }", "  catch (err) {",
                "    console.error('Cannot determine image background color for ' + img.src, err)",
                "    $('canvas').remove();", "  }", "}", "", "function fallbackToNonCors(img) {",
                "  $(img).removeAttr('crossorigin');", "  var x = img.src;", "  img.src = '';",
                "  img.src = x;", "  var warning = `",
                ...
                "        </i>", "      </span>", "    </div>", "  `",
                "  $(warning).insertAfter(img);", "}", "var startTime = new Date().getTime();",
                ...
                "      fallbackToNonCors(img);", "    }", "    else {",
                "      updateImageBackground(img);", "    }", "  }", "  else {",
