...
public class StatementEditor<T extends Serializable> extends Panel {
    ...
    private IModel<KBStatement<T>> statement;
    private Component content;

    public StatementEditor(String aId, IModel<KnowledgeBase> aKbModel,
            IModel<KBStatement<T>> aStatement) {
    ...
    protected void actionEdit(AjaxRequestTarget aTarget) {
        ...
        KBStatement<T> shallowCopy = new KBStatement<T>(statement.getObject());
        IModel<KBStatement<T>> shallowCopyModel = Model.of(shallowCopy);
    ...
    private void actionSave(AjaxRequestTarget aTarget, Form<KBStatement<T>> aForm) {
        KBStatement<T> modifiedStatement = aForm.getModelObject();
    ...
    private class ViewMode extends Fragment {
        ...
        public ViewMode(String aId, IModel<KBStatement<T>> aStatement) {
            super(aId, "viewMode", StatementEditor.this, aStatement);

            CompoundPropertyModel<KBStatement<T>> compoundModel = new CompoundPropertyModel<>(
                    aStatement);
            ...
            add(new LambdaAjaxLink("edit", StatementEditor.this::actionEdit)
                    .onConfigure((_this) -> _this.setVisible(!statement.getObject().isInferred())));
            add(new LambdaAjaxLink("makeExplicit", StatementEditor.this::actionMakeExplicit)
                    .onConfigure((_this) -> _this.setVisible(statement.getObject().isInferred())));
        }
    }
    ...
    private class EditMode extends Fragment implements Focusable {
         ...
         */
        public EditMode(String aId, IModel<KBStatement<T>> aStatement, boolean isNewStatement) {
            super(aId, "editMode", StatementEditor.this, aStatement);

            Form<KBStatement<T>> form = new Form<>("form", CompoundPropertyModel.of(aStatement));

            // text area for the statement value should receive focus
            Component valueTextArea = new TextArea<T>("value").add(
