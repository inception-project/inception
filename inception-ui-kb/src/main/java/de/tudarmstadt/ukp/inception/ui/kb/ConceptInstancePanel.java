...
public class ConceptInstancePanel extends EventListeningPanel {
    ...
    public ConceptInstancePanel(String aId, IModel<KnowledgeBase> aKbModel,
        ...
        eventHandler.addCallback(AjaxInstanceSelectionEvent.class,
                this::actionInstanceSelection);
    ...
    private void actionStatementChanged(AjaxRequestTarget target,
        ...
        boolean isRenameEvent = statement.getProperty()
                .getIdentifier()
    ...
    private void actionInstanceSelection(AjaxRequestTarget target,
            AjaxInstanceSelectionEvent event) {
        ...
        if (selectedInstanceHandle.getObject() != null) {
            ...
            KBInstance selectedInstance = kbService.readInstance(kbModel.getObject(),
                    selectedInstanceHandle.getObject().getIdentifier());
            replacementPanel = new InstanceInfoPanel(INSTANCE_INFO_MARKUP_ID, kbModel,
                    selectedInstanceHandle, Model.of(selectedInstance));
        } else {
            replacementPanel = new EmptyPanel(INSTANCE_INFO_MARKUP_ID).setVisibilityAllowed(false);
        }
    ...
    private void actionNewInstance(AjaxRequestTarget target, AjaxNewInstanceEvent event) {
        ...
        URI type = selectedConceptHandle.getObject() != null
                ? URI.create(selectedConceptHandle.getObject().getIdentifier()) : null;
