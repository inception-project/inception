 ...
 * {@link AnnotationGuideLinePanel} is used to update documents to a project.
 * The {@link ProjectDetailsPanel} used for updating Project deatils such as
 * descriptions of a project and name of the Project The
...
public class ProjectPage extends SettingsPageBase {
	...
	public ProjectPage() {
		...
		MetaDataRoleAuthorizationStrategy.authorize(importProjectForm,
				Component.RENDER, "ROLE_ADMIN");
	}

	class ProjectSelectionForm extends Form<SelectionModel> {
		...
		public ProjectSelectionForm(String id) {
			...
			MetaDataRoleAuthorizationStrategy.authorize(creatProject,
					Component.RENDER, "ROLE_ADMIN");
	...
	private class ImportProjectForm extends Form<Void> {
					...
					for (FileUpload exportedProject : exportedProjects) {
						...
						try {
							...
							for (Enumeration zipEnumerate = zip.entries(); zipEnumerate
									.hasMoreElements();) {
								ZipEntry entry = (ZipEntry) zipEnumerate
										.nextElement();
								if (entry
										.toString()
										.replace("/", "")
										.startsWith(
												ProjectUtil.EXPORTED_PROJECT)
										&& entry.toString().replace("/", "")
												.endsWith(".json")) {
									projectInputStream = zip
							...
							String text = IOUtils.toString(projectInputStream,
									"UTF-8");
							...
							/*
							 * for (TagSet tagset :
							 * importedProjectSetting.getTagSets()) {
							...
							ProjectUtil.createProjectGuideline(zip,
									importedProject, repository);
							// cretae project META-INF
							ProjectUtil.createProjectMetaInf(zip,
									importedProject, repository);
						} catch (IOException e) {
	...
	public class ProjectDetailForm extends Form<Project> {
		private static final long serialVersionUID = -1L;
		...
		public ProjectDetailForm(String id) {
			...
			add(allTabs = (AjaxTabbedPanel) new AjaxTabbedPanel<ITab>("tabs",
	...
	private class ProjectDetailsPanel extends Panel {
		...
		public ProjectDetailsPanel(String id) {
			...
			add(projectType = (RadioChoice<Mode>) new RadioChoice<Mode>("mode",
					Arrays.asList(new Mode[] { Mode.ANNOTATION,
							Mode.AUTOMATION, Mode.CORRECTION }))
			...
			add(new Button("save", new ResourceModel("label")) {
				...
				public void onSubmit() {
					...
					if (!ProjectUtil.isNameValid(project.getName())) {
						...
						if (project.getId() != 0) {
							project.setName(ProjectUtil.validName(project
									.getName()));
						}
					...
					if (repository.existsProject(project.getName())
							&& project.getId() == 0) {
						error("Another project with name [" + project.getName()
								+ "] exists");
						return;
					}
				...
				public void onSubmit() {
					...
					try {
						...
						repository.removeProject(
								projectDetailForm.getModelObject(), user);
						projectDetailForm.setVisible(false);
					} catch (IOException e) {
