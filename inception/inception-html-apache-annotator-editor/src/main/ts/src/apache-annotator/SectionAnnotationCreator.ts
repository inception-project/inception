...
export class SectionAnnotationCreator {
    ...
    private handleIntersect(
        ...
        this.observerDebounceTimeout = window.requestAnimationFrame(() => {
            ...
            for (const entry of entries) {
                ...
                let panel = this.root.querySelector(
                    `.iaa-section-control[data-iaa-applies-to="${sectionId}"]`
                ) as HTMLElement;
    ...
    private createAnnotation(event: MouseEvent) {
        ...
        this.ajax.createSpanAnnotation([
            [calculateStartOffset(this.root, section), calculateEndOffset(this.root, section)],
        ]);
    }
    ...
    private showPreviewFrame() {
        ...
        this._previewFrame.addEventListener('mouseleave', (event) =>
            this.removeSectionHighight(event)
        );
    ...
    private renderPreviewFrame() {
        ...
        copy.querySelectorAll(
            '.iaa-preview-frame, .iaa-section-control, ' +
                '.iaa-visible-annotations-panel, .iaa-visible-annotations-panel-spacer'
        ).forEach((n) => n.remove());

        const block = this._previewFrame.contentDocument?.querySelector(
            '[data-iaa-section-control-hover]'
        );
        if (!block) return;
