...
export class SectionAnnotationVisualizer {
    ...
    clear() {
        if (this.sectionSelector) {
            this.root.parentElement
                ...
                .forEach((e) => e.remove());
            this.root
                .querySelectorAll('.iaa-visible-annotations-panel-spacer')
                .forEach((e) => e.remove());
        }
    }
    ...
    private ensurePanelVisibility(reason: string) {
        ...
        const panels = Array.from(
            this.root.parentNode?.querySelectorAll('.iaa-visible-annotations-panel') || []
        );
        const root = this.root.closest('.i7n-wrapper') || this.root;
        ...
        performance.measure(
            'SectionAnnotationVisualizer.ensurePanelVisibility.init',
            'start-ensure-panel-visibility-init',
            'end-ensure-panel-visibility-init'
        );
        ...
        performance.measure(
            'SectionAnnotationVisualizer.getSectionSpacers',
            'start-get-section-spacers',
            'end-get-section-spacers'
        );
        ...
        for (const panel of panels as HTMLElement[]) {
            ...
            try {
                ...
                if (sectionLeavingViewport) {
                    const hiddenUnderHigherLevelPanel =
                        lastSectionPanelBottom &&
                        sectionRect.bottom + rootTop - spacerRect.height < lastSectionPanelBottom;
                ...
                const shouldKeepPanelVisibleAtTop =
                    spacerRect.top < lastSectionPanelBottom &&
                    !(sectionRect.bottom < lastSectionPanelBottom);
                ...
                performance.measure(
                    `SectionAnnotationVisualizer.renderSectionPanel-${sectionId}`,
                    `start-render-section-panel-${sectionId}`,
                    `end-render-section-panel-${sectionId}`
                );
            }
        }
        performance.mark('end-render-section-panels');
        performance.measure(
            'SectionAnnotationVisualizer.renderSectionPanels',
            'start-render-section-panels',
            'end-render-section-panels'
        );
        ...
        performance.measure(
            `SectionAnnotationVisualizer.ensurePanelVisibility (${reason})`,
            'start-ensure-panel-visibility',
    ...
    private renderSectionGroups(doc: AnnotatedText) {
        ...
        for (const [vid, sectionElement] of sectionElements) {
            ...
            if (!panel) {
                if (!sectionElement.id) {
                    console.warn(
                        `Cannot create section panel for section without ID`,
                        sectionElement
                    );
                    continue;
                }
        ...
        performance.measure(
            'SectionAnnotationVisualizer.createAnnotationPanels',
            'start-create-annotation-panels',
            'end-create-annotation-panels'
        );
        ...
        for (const panel of panels as HTMLElement[]) {
            ...
            performance.measure(
                `SectionAnnotationVisualizer.createSpacer-${appliesTo}`,
                `start-create-spacer-${appliesTo}`,
                `end-create-spacer-${appliesTo}`
            );
        }
        performance.mark('end-create-spacers');
        performance.measure(
            'SectionAnnotationVisualizer.createSpacers',
            'start-create-spacers',
            'end-create-spacers'
        );
        ...
        performance.measure(
            'SectionAnnotationVisualizer.insertSpacers',
            'start-insert-spacers',
            'end-insert-spacers'
        );
    }
    ...
    private addAnnotationPanel(panel: Element) {
        if (this.root) {
            ...
            console.error(
                'Parent element of root element not found - cannot add visible annotations panel'
            );
        }
    }
}
