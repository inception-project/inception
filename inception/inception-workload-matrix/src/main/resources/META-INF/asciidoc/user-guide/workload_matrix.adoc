...
The *Curation* column also indicates whether a document is ready to be curated.
Under static assignment, a document is considered ready for curation once every annotator has either *finished* or *locked* it.
If curation has been started on a document whose annotation is not yet complete, the cell is highlighted to make this visible, since curating now means curating incomplete data.
Curation is still permitted in this case - the indicator is only a hint (see also the readiness badge described in <<sect_curation>>).
