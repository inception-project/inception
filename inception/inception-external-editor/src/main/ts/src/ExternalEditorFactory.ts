...
export class ExternalEditorFactory implements AnnotationEditorFactory {
    async getOrInitialize(
        ...
        if (element[PROP_INITIALIZING] != null) {
            console.debug(
                '[getOrInitialize] Initialization already in progress, waiting for existing promise'
            );
        ...
        if (element instanceof HTMLIFrameElement) {
            ...
            element[PROP_INITIALIZING] = this.loadIFrameContent(iframe)
                .then((win) => {
                    console.debug(
                        '[getOrInitialize] loadIFrameContent resolved, calling loadEditorResources'
                    );
                    return this.loadEditorResources(win, props);
                })
                .then((win) => {
                    console.debug(
                        '[getOrInitialize] loadEditorResources resolved, calling installKeyEventForwarding'
                    );
                    return this.installKeyEventForwarding(win, element.ownerDocument.defaultView);
                })
                .then((win) => {
                    console.debug(
                        '[getOrInitialize] installKeyEventForwarding resolved, calling initEditor'
                    );
    ...
    loadIFrameContent(iframe: HTMLIFrameElement): Promise<Window> {
        return new Promise((resolve) => {
            ...
            const eventHandler = () => {
                ...
                console.debug(
                    `[loadIFrameContent] Load event fired - document location: ${content?.location}, readyState: ${content?.readyState}`
                );
    ...
    loadEditorResources(win: Window, props: AnnotationEditorProperties): Promise<Window> {
        return new Promise((resolve) => {
            console.debug(
                `[loadEditorResources] Starting - document location: ${win.document.location}, readyState: ${win.document.readyState}`
            );
            ...
            if (this.isDocumentStylesheetCapable(win.document) && props.stylesheetSources) {
                allPromises.push(
                    ...props.stylesheetSources.map((src) => this.loadStylesheet(target, src))
                );
            }

            if (props.scriptSources) {
                console.debug(
                    `[loadEditorResources] Loading ${props.scriptSources.length} script(s)`
                );
            ...
            Promise.all(allPromises).then(() => {
                console.info(
                    `[loadEditorResources] ${allPromises.length} editor resources loaded - resolving`
                );
                resolve(win);
            });
        });
    ...
    initEditor(
        ...
        return new Promise((resolve) => {
            console.debug(
                `[initEditor] Starting - document location: ${contextWindow.document.location}, readyState: ${contextWindow.document.readyState}`
            );

            const editorFactory = (contextWindow as any).eval(
                props.editorFactory
            ) as AnnotationEditorFactory;
    ...
    loadStylesheet(document: Document, styleSheetSource: string): Promise<void> {
        return new Promise((resolve) => {
            ...
            if (headElements.length > 0) {
                ...
                console.warn(
                    `Unable to register stylesheet: ${styleSheetSource} - no head element found`
                );
                resolve();
            }
        });
