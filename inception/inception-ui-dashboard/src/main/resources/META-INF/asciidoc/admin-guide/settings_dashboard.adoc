...
| `dashboard.transforming-export.enabled` 🧪
| Enable the experimental *Transforming project export* option on the project export page, which
  allows applying transformations such as text obfuscation to source documents during export
| `false`
| `true`
|===
