...
1. *State:* state of the document within the project.
   This column also indicates readiness for curation: if curation has been started on a document whose annotation is not yet complete, the cell is highlighted.
   Under dynamic assignment, a document is considered ready for curation once the required number of annotators (see *Annotators per document* in the <<Dynamic workload settings>>) have marked it as *finished*.
