<script lang="ts">
    ...
    function getActivity(week: number, dayOfWeek: number) {
        ...
        return item
            ...
            : {
                  date: dateStr,
                  count: 0,
                  outOfRange: date.isBefore(overviewData.from) || date.isAfter(overviewData.to),
              };
    }
    ...
    function calculateDate(week: number, dayOfWeek: number) {
        ...
        const date = firstDayOfYear.add(week * 7 + dayOfWeek - firstDayDayOfWeek, 'day');
        return date;
    }
                    ...
                    <tbody>
                        <tr>
                            ...
                            {#each { length: 53 } as _, week}
                                {@const monthStartingInWeek = getMonthStartingInWeek(week)}
                                {#if monthStartingInWeek}
                                    <td class="col-legend text-nowrap">{monthStartingInWeek}</td>
                                {:else}
                                    <td></td>
                                {/if}
                            {/each}
                        ...
                        {#each { length: 7 } as _, dayOfWeek}
                            <tr>
                                ...
                                {#each { length: 53 } as _, week}
                                    ...
                                    <td
                                        class="cell"
                                        class:highlight={!item?.outOfRange &&
                                            selectedRange.from &&
                                        ...
                                        style:visibility={item?.outOfRange ? 'hidden' : 'visible'}
...
{:else}
    {#if processedSummaryData}
        <div class="d-flex flex-row justify-content-between small text-muted m-2">
            <div>
                {dayjs(processedSummaryData.from).format('LL')} - {dayjs(
                    processedSummaryData.to
                ).format('LL')}
            </div>
            <div>
    ...
    {:else if processedSummaryData}
        <div class="list-group m-2">
            ...
            {#if processedSummaryData.globalItems && processedSummaryData.globalItems.length > 0}
                <div class="list-group-item d-flex justify-content-between align-items-start">
                    <div class="ms-1 me-auto">
                        <div class="fw-bold text-break">Project-level activities</div>
