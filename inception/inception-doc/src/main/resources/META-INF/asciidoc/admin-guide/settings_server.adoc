...
| `server.address`
| IP address on which the server listens. When running from the desktop
  application bundle (Windows MSI / macOS DMG), this defaults to
  `127.0.0.1` so the application is reachable only from the local
  machine. See <<user-guide.adoc#sect_installation_app_bundle, Running as a desktop application>> in the User Guide for details.
| `0.0.0.0` _(JAR / WAR);_ `127.0.0.1` _(desktop bundle)_
| `127.0.0.1`
