...
We use a style for formatting the source code in {product-name}. Our approach consists of three
parts:

* DKPro code formatting profile - the profile configures your IDE to auto-format the code according
  to our guidelines as you go.
* Spotless - applies the DKPro formatter, removes unused imports and ensures files end with a
  newline. Run `mvn -Pspotless spotless:apply` to auto-format the code, or
  `mvn -Pspotless spotless:check` to verify it. The check is part of the Maven `verify` phase and
  the build fails if the code is not formatted properly.
* Checkstyle - enforces structural and naming rules that Spotless does not cover (e.g. star
  imports, package and type-parameter naming, empty catch blocks, `equals`/`hashCode` consistency).
  It is also run as part of the Maven `verify` phase.
...
The repository ships a pre-commit hook in `.githooks/pre-commit` that runs `spotless:check` and
`checkstyle:check` against the Maven modules containing staged Java files. The first time you run
any Maven build (e.g. `mvn install`) the `git-build-hook-maven-plugin` configures git to use this
hooks directory by setting `core.hooksPath` automatically — no manual setup is required.

The hook is a no-op when no Java files are staged. If a check fails, run
`mvn -Pspotless spotless:apply` to auto-fix formatting issues, re-stage the changes and commit
again. As a last resort you can bypass the hook for a single commit with `git commit --no-verify`.

To disable the hook entirely, unset `core.hooksPath` in your local git config.
