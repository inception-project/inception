 ...
 *
 * The Process API represents the communication end-point between client (editor) and server
 * (back-end). All data sent by any of the clients via websocket will be retrieved here.
 *
 * Therefore, each receive() method have a '@MessageMapping' annotation for a certain topic. The
 * topics can be found
 * 
 * @see AnnotationProcessAPIImpl
 *
 *      Every send() method ends with 'simpMessagingTemplate.convertAndSend()'. This sends the
 *      message to a certain topic with a payload which contains always a 'JSONUtil.toJsonString()'
 *      representation of a message from the '../messages' package
 *
 *      LOG.debug() has been added for all of the methods in order to better track incoming and
 *      outgoing data.
 *
 *      NOTE: Not every sending of a message must precede an incoming message. The server can send a
 *      message to ANY client without the client requesting something. Event Types created on the
 *      server-side in 'inception-api-annotation/src/main/java/../../event/*'
 *
...
public interface AnnotationProcessAPI
{
    ...
    void sendDocumentResponse(DocumentMessage aDocumentResponse, String aUser) throws IOException;
    ...
    void sendCreateArc(ArcCreatedMessage aCreateArcMessage, String aProjectID, String aDocumentID)
        throws IOException;
