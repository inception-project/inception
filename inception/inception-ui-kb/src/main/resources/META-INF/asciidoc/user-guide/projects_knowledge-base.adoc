...
* link:https://graphdb.ontotext.com/documentation/11.3/full-text-search.html[GraphDB]
* link:https://github.com/ad-freiburg/qlever/blob/master/docs/sparql_plus_text.md[QLever (SPARQL+Text)]
...
In GraphDB, the languages for full-text indexing need to be explicitly configured.
E.g. if you have string literals without a language tag (`default`), with an English language tag (`en`) and with a
German language tag (`de`) in your KB, you need to configure full-text indices for all of these:

```
<http://www.ontotext.com/config/graphdb#fts-indexes> ("default" "de" "en");
```

{product-name} expects that knowledge bases do not perform any type of inference.
To run GraphDB in the way we test it, you need to disable inference via an empty ruleset:
...
link:https://github.com/ad-freiburg/qlever[QLever] is a SPARQL engine that integrates a separate
full-text index, queried via the magic predicates `ql:contains-entity` and `ql:contains-word`. This
support targets QLever instances whose text index was built from the literals of the knowledge base,
so that entities can be matched by their label. The public
link:https://sparql.dblp.org/sparql[dblp endpoint] is such an instance and is available as the
built-in **DBLP Computer Science Bibliography** knowledge base profile.

Note that QLever evaluates `REGEX` using Google's link:https://github.com/google/re2[RE2] library,
which does not support look-ahead assertions. {product-name} takes this into account when generating
the queries for QLever.
