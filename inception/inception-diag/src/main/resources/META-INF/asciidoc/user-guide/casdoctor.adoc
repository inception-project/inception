...
[[check_AllAnnotationsWithinDocumentTextCheck]]
=== All annotations within the document text
[horizontal]
ID:: `AllAnnotationsWithinDocumentTextCheck`
Related repairs:: <<repair_ClampAnnotationsToDocumentTextRepair>>

Checks if all annotations lie within the boundaries of the document.
...
[[repair_ClampAnnotationsToDocumentTextRepair]]
=== Clamp annotations to the document text

[horizontal]
ID:: `ClampAnnotationsToDocumentTextRepair`

This repair clamps annotations whose offsets lie outside the document text back into it.
Annotations that lie entirely after the end of the document text are collapsed into a zero-width
annotation at the end of the text. Annotations starting before the beginning of the text are
clamped to start at zero.
The document text itself is not modified - only the offsets of the affected annotations are.

NOTE: Run the checks again after applying this repair as certain annotations can become invalid if
      they get clamped down to a length of zero. It may be necessary to apply another repair such as
      <<repair_RemoveZeroSizeTokensAndSentencesRepair>> to remove these annotations.
...
NOTE: Run the checks again after applying this repair. The document text becomes one character
      shorter, so any annotation whose offsets were not adjusted now reaches beyond the end of the
      document text - see <<check_AllAnnotationsWithinDocumentTextCheck>> and
      <<repair_ClampAnnotationsToDocumentTextRepair>>.
