...
export class RecogitoEditor implements AnnotationEditor {
    ...
    public constructor(element: Element, ajax: DiamAjax, userPreferencesKey: string) {
        ...
        ajax.loadPreferences(userPreferencesKey)
            ...
            .then(() => {
                ...
                this.recogito.on('selectAnnotation', (annotation) =>
                ...
                element.addEventListener('mousedown', (e) => this.cancelRightClick(e), {
                    capture: true,
                });
                element.addEventListener('mouseup', (e) => this.cancelRightClick(e), {
                    capture: true,
                });
                element.addEventListener('mouseclick', (e) => this.cancelRightClick(e), {
                    capture: true,
                });

                this.installSpanRenderingPatch(this.recogito);

                this.connections = ConnectionsPlugin(this.recogito, {
                    disableEditor: true,
                    showLabels: annotatorState.showLabels,
                });
                this.connections.canvas.on('createConnection', (annotation) =>
                    this.createRelation(annotation)
                );
                this.connections.canvas.on('selectConnection', (annotation) =>
                    this.selectAnnotation(annotation)
                );
                ...
                this.root.ownerDocument.body.addEventListener('mouseover', (e) =>
                    this.addAnnotationHighlight(e as MouseEvent)
                );
                this.root.ownerDocument.body.addEventListener('mouseout', (e) =>
                ...
                for (const annotation of annotations) {
                    for (const element of this.root.querySelectorAll(
                        ...
                        if (element instanceof SVGElement) {
                            annotation.body.classes.forEach((c) => element.classList.add(c));
                            element
                                .querySelectorAll('.r6o-connections-edge-path-inner')
                                .forEach((path) => {
    ...
    private convertAnnotations(doc: AnnotatedText) {
        ...
        console.info(
            `Loaded ${webAnnotations.length} annotations from server (${doc.spans?.size || 0} spans and ${doc.relations?.size || 0} relations)`
        );
    }
