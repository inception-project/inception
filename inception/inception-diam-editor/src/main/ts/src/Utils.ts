...
export function uniqueLayers(data: AnnotatedText): Layer[] {
    ...
    const sortedLayersWithDuplicates = Array.from(data.annotations(), (ann) => ann.layer).sort(
        (a, b) => a.name.localeCompare(b.name, undefined, { usage: 'sort', sensitivity: 'variant' })
    );
...
export function uniqueLabels(data: AnnotatedText): string[] {
    ...
    const sortedLabelsWithDuplicates = Array.from(data.annotations(), (ann) =>
        renderLabel(data, ann)
...
export function uniqueOffsets(data: AnnotatedText): Offsets[] {
    ...
    const sortedOffsetsWithDuplicates = Array.from(
        data.spans.values(),
        (span) => span.offsets[0]
    ).sort(compareOffsets);
    ...
    for (let i = 0; i < sortedOffsetsWithDuplicates.length; i++) {
        if (
            i === 0 ||
            compareOffsets(sortedOffsetsWithDuplicates[i - 1], sortedOffsetsWithDuplicates[i]) !== 0
        ) {
...
export function groupSpansByLabel(data: AnnotatedText): Record<string, Span[]> {
    ...
    for (const items of Object.values(groups)) {
        items.sort(
            (a, b) => compareSpanText(data, a, b) || compareOffsets(a.offsets[0], b.offsets[0])
        );
    }
    return groups;
}
...
export function filterAnnotations(data: AnnotatedText, items: Annotation[], filter: string) {
    ...
    return items.filter((item) => {
        if (item instanceof Relation) {
            return getCoveredText(data, item.arguments[0].target)
                .replace(/\s+/g, ' ')
                .toLowerCase()
                .includes(normalizedFilter);
        }
        if (item instanceof Span) {
            return getCoveredText(data, item)
                .replace(/\s+/g, ' ')
                .toLowerCase()
                .includes(normalizedFilter);
        }
