...
====
NOTE: Document-level annotations can be curated in the **Document Metadata sidebar** when the
project uses the *integrated curation mode*. Document-level annotation curation is not available
in the *split curation mode*. Import and export of document-level annotations is only supported in
the UIMA CAS XMI and UIMA CAS JSON formats.
====
