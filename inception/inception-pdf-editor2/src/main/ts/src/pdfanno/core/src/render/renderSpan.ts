...
export function mapToDocumentCoordinates(aRectangles: Rectangle[]): Rectangle[] | undefined {
    ...
    for (let r of aRectangles) {
        ...
        r = new Rectangle({
            p: r.p,
            x: xCorrectionFactor * r.x + pageLeftX,
            y: yCorrectionFactor * r.y + pageTopY,
            w: r.w,
            h: r.h,
        });
...
export function mergeRects(glyphs: VGlyph[]): Rectangle[] {
    ...
    for (let i = 1; i < glyphs.length; i++) {
        ...
        if (
            aggregateRect.p === glyphBox.p &&
            withinMargin(glyphBox.top, aggregateRect.top, error)
        ) {
