...
export class PdfAnnotationEditor implements AnnotationEditor {
    ...
    public constructor(element: Element, ajax: DiamAjax) {
        ...
        this.root.addEventListener('createRelationAnnotation', (ev) =>
            this.onCreateRelationAnnotation(ev)
        );
        this.root.addEventListener('doubleClickAnnotation', (ev) =>
            this.onDoubleClickAnnotation(ev)
        );
        this.root.addEventListener('openContextMenu', (ev) => this.onOpenContextMenu(ev));
    }

    async init(): Promise<void> {
        return initPdfAnno(this.ajax).then(() =>
