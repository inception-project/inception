...
import {
    ...
    VID,
    LazyDetailGroup,
    DiamAjaxConnectOptions,
    DiamLoadAnnotationsOptions,
    DiamSelectAnnotationOptions,
...
export class DiamAjaxImpl implements DiamAjax {
            ...
            Wicket.Ajax.ajax({
                ...
                eh: [
                    () => {
                        ...
                        reject(new Error('Unable to load annotations'));
                    },
                ],
            });
        });
            ...
            Wicket.Ajax.ajax({
                ...
                eh: [
                    () => {
                        ...
                        reject(new Error('Unable to refresh annotations'));
                    },
                ],
            });
        });
    ...
    loadLazyDetails(
        idOrAnnotation: VID | Annotation,
        optionaLayerId?: number
    ): Promise<LazyDetailGroup[]> {
        const token = DiamAjaxImpl.newToken();
    ...
    private calculateClientPosition(evt: MouseEvent | null): {
        clientX: number;
        clientY: number;
        overlay?: HTMLElement;
    } {
    ...
    private closeOverlayWhenContextMenuIsHidden(overlay?: HTMLElement): void {
        ...
        const contextMenu = overlay.parentElement?.querySelector(
            '.context-menu'
        ) as HTMLElement | null;
        if (!contextMenu) {
            return;
