...
| Gamma (γ)
| Aligning
| Chance-corrected inter-rater agreement for multiple raters following Mathet et al. (2015).
The annotations of the raters are aligned such that the overall disagreement (disorder) with respect to position and label is minimal.
The expected disorder is estimated by sampling random annotation sets from the statistical characteristics of the actual annotations (cf. pygamma-agreement).
Note that the sampling is performed with a fixed seed, so repeated calculations on the same data yield the same result.
Zero-width annotations are not supported and skipped.
|====
...
.Aligning measures
Aligning measures (e.g. gamma) are related to unitizing measures in that they also operate on character offsets instead of fixed positions.
However, instead of considering all possible unitizations, they search for an optimal alignment between the annotations of the different annotators - i.e. an alignment that minimizes the overall disagreement (disorder) with respect to position and label.
The observed disorder is then set into relation to the disorder expected by chance, which is estimated using a randomized (Monte-Carlo) procedure.
Relation and chain layers are presently not supported by the aligning measures.
