...
public class MtasSearchTestConsistency {
	...
	public static void initialize() {
		try {
			...
			files.put("Een oude kennis",
					dataPath.resolve("resources").resolve("beets2.xml.gz").toAbsolutePath().toString());
			files.put("Varen en Rijden",
					dataPath.resolve("resources").resolve("beets3.xml.gz").toAbsolutePath().toString());
	...
	public void basicSearchNumberOfWords() throws IOException {
		IndexReader indexReader = DirectoryReader.open(directory);
		testNumberOfHits(indexReader, FIELD_CONTENT, Arrays.asList("[]"), Arrays.asList("[][]", "[#0]"));
		indexReader.close();
	}
	...
	public void basicSearchStartSentence2() throws IOException {
		IndexReader indexReader = DirectoryReader.open(directory);
		testNumberOfHits(indexReader, FIELD_CONTENT, Arrays.asList("[]</s><s>[]", "[#0]"), Arrays.asList("<s>[]"));
		indexReader.close();
	}
	...
	public void basicSearchIntersecting1() throws IOException {
		...
		testNumberOfHits(indexReader, FIELD_CONTENT, Arrays.asList("<s/>"),
				Arrays.asList("<s/> intersecting [pos=\"ADJ\"]", "<s/> !intersecting [pos=\"ADJ\"]"));
		indexReader.close();
	}
	...
	public void basicSearchIntersecting2() throws IOException {
		...
		// get total number
		IndexReader indexReader = DirectoryReader.open(directory);
		QueryResult queryResult1 = doQuery(indexReader, FIELD_CONTENT, cql, null, null, null, false);
		QueryResult queryResult1disabled = doQuery(indexReader, FIELD_CONTENT, cql, null, null, null, true);
		assertFalse("Intersecting: " + cql + " has no hits (" + queryResult1.hits + ")", queryResult1.hits == 0);
		assertEquals("Intersecting: - twoPhaseIterator", queryResult1.hits, queryResult1disabled.hits);
	}
	...
	public void basicSearchIntersecting3() throws IOException {
		...
		QueryResult queryResult1 = doQuery(indexReader, FIELD_CONTENT, cql, null, null, null, false);
		QueryResult queryResult1disabled = doQuery(indexReader, FIELD_CONTENT, cql, null, null, null, true);
		assertTrue("Intersecting: " + cql + " has hits (" + queryResult1.hits + ")", queryResult1.hits == 0);
		assertEquals("Intersecting: - twoPhaseIterator", queryResult1.hits, queryResult1disabled.hits);
	}
	...
	public void basicSearchIgnore() throws IOException {
		...
		IndexReader indexReader = DirectoryReader.open(directory);
		QueryResult queryResult1 = doQuery(indexReader, FIELD_CONTENT, cql1, null, null, null, false);
		QueryResult queryResult1disabled = doQuery(indexReader, FIELD_CONTENT, cql1, null, null, null, true);
		...
		QueryResult queryResult2 = doQuery(indexReader, FIELD_CONTENT, cql2, ignore, ignoreNumber, null, false);
		QueryResult queryResult2disabled = doQuery(indexReader, FIELD_CONTENT, cql2, ignore, ignoreNumber, null, true);
		assertEquals("Article followed by Noun ignoring Adjectives", queryResult1.hits, queryResult2.hits);
	...
	public void basicSearchSequence() throws IOException {
		...
		QueryResult queryResult1 = doQuery(indexReader, FIELD_CONTENT, cql1, null, null, null, false);
		QueryResult queryResult1disabled = doQuery(indexReader, FIELD_CONTENT, cql1, null, null, null, true);
		QueryResult queryResult2 = doQuery(indexReader, FIELD_CONTENT, cql2, null, null, null, false);
		QueryResult queryResult2disabled = doQuery(indexReader, FIELD_CONTENT, cql2, null, null, null, true);
		QueryResult queryResult3 = doQuery(indexReader, FIELD_CONTENT, cql3, null, null, null, false);
		QueryResult queryResult3disabled = doQuery(indexReader, FIELD_CONTENT, cql3, null, null, null, true);
		assertTrue(
				"Sequences: not #" + cql1 + " (" + queryResult1.hits + ") >= #" + cql2 + " (" + queryResult2.hits + ")",
				queryResult1.hits >= queryResult2.hits);
		assertTrue("Sequences: not #" + cql1 + " (" + queryResult1.hits + ") >= #" + cql2 + " (" + queryResult2.hits
				+ ") - disabled twoPhaseIterator", queryResult1disabled.hits >= queryResult2disabled.hits);
		assertTrue(
				"Sequences: not #" + cql1 + " (" + queryResult1.hits + ") >= #" + cql3 + " (" + queryResult3.hits + ")",
				queryResult1.hits <= queryResult3.hits);
		assertTrue("Sequences: not #" + cql1 + " (" + queryResult1.hits + ") >= #" + cql3 + " (" + queryResult3.hits
				+ ") - disabled twoPhaseIterator", queryResult1disabled.hits <= queryResult3disabled.hits);
	...
	public void basicSearchWithin1() throws IOException {
		IndexReader indexReader = DirectoryReader.open(directory);
		testNumberOfHits(indexReader, FIELD_CONTENT, Arrays.asList("[]"), Arrays.asList("[] within <s/>"));
		indexReader.close();
	}
	...
	public void basicSearchWithin2() throws IOException {
		...
		QueryResult queryResult3 = doQuery(indexReader, FIELD_CONTENT, cql3, null, null, null, false);
		QueryResult queryResult3disabled = doQuery(indexReader, FIELD_CONTENT, cql3, null, null, null, true);
		assertFalse("Within: " + cql3 + " has no hits (" + queryResult3.hits + ")", queryResult3.hits == 0);
		assertEquals("Within: " + cql3, queryResult3.hits, (long) queryResult1.hits + queryResult2.hits);
	...
	public void basicSearchWithin3() throws IOException {
		...
		QueryResult queryResult3disabled = doQuery(indexReader, FIELD_CONTENT, cql3, null, null, null, true);
		assertFalse("Within: " + cql3 + " has no hits (" + queryResult3.hits + ")", queryResult3.hits == 0);
		assertTrue("Within: " + cql3 + " not " + queryResult3.hits + " >= " + queryResult1.hits + " + "
				+ queryResult2.hits, queryResult3.hits >= queryResult1.hits + queryResult2.hits);
		assertTrue("Within: " + cql3 + " not " + queryResult3.hits + " >= " + queryResult1.hits + " + "
	...
	public void basicSearchWithin4() throws IOException {
		...
		QueryResult queryResult1disabled = doQuery(indexReader, FIELD_CONTENT, cql, null, null, null, true);
		assertTrue("Within: " + cql + " has hits (" + queryResult.hits + ")", queryResult.hits == 0);
	...
	public void basicSearchWithin5() throws IOException {
		...
		QueryResult queryResult1 = doQuery(indexReader, FIELD_CONTENT, cql, null, null, null, false);
		QueryResult queryResult1disabled = doQuery(indexReader, FIELD_CONTENT, cql, null, null, null, true);
		assertFalse("Within: " + cql + " has no hits (" + queryResult1.hits + ")", queryResult1.hits == 0);
	...
	public void basicSearchContaining2() throws IOException {
		...
		QueryResult queryResult3 = doQuery(indexReader, FIELD_CONTENT, cql3, null, null, null, false);
		QueryResult queryResult3disabled = doQuery(indexReader, FIELD_CONTENT, cql3, null, null, null, true);
		QueryResult queryResult4 = doQuery(indexReader, FIELD_CONTENT, cql4, null, null, null, false);
		QueryResult queryResult4disabled = doQuery(indexReader, FIELD_CONTENT, cql4, null, null, null, true);
		assertEquals("Containing: " + cql3, queryResult3.hits, queryResult2.hits);
		assertEquals("Containing: " + cql3 + " disabled twoPhaseIterator", queryResult3disabled.hits,
				queryResult2disabled.hits);
		assertEquals("Containing: " + cql4, queryResult4.hits, (long) queryResult1.hits - queryResult2.hits);
	...
	public void basicSearchContaining3() throws IOException {
		...
		QueryResult queryResult1 = doQuery(indexReader, FIELD_CONTENT, cql, null, null, null, false);
		QueryResult queryResult1disabled = doQuery(indexReader, FIELD_CONTENT, cql, null, null, null, true);
		assertTrue("Containing: " + cql + " has hits (" + queryResult1.hits + ")", queryResult1.hits == 0);
	...
	public void basicSearchContaining4() throws IOException {
		...
		QueryResult queryResult1 = doQuery(indexReader, FIELD_CONTENT, cql, null, null, null, false);
		QueryResult queryResult1disabled = doQuery(indexReader, FIELD_CONTENT, cql, null, null, null, true);
		assertFalse("Containing: " + cql + " has no hits (" + queryResult1.hits + ")", queryResult1.hits == 0);
		assertEquals("Containing: - twoPhaseIterator", queryResult1.hits, queryResult1disabled.hits);
	}
	...
	public void basicSearchFollowedBy1() throws IOException {
		...
		QueryResult queryResult3 = doQuery(indexReader, FIELD_CONTENT, cql3, null, null, null, false);
		QueryResult queryResult3disabled = doQuery(indexReader, FIELD_CONTENT, cql3, null, null, null, true);
		assertEquals("Article followed by Adjective", queryResult1.hits, (long) queryResult2.hits - queryResult3.hits);
	...
	public void basicSearchPrecededBy1() throws IOException {
		...
		QueryResult queryResult3 = doQuery(indexReader, FIELD_CONTENT, cql3, null, null, null, false);
		QueryResult queryResult3disabled = doQuery(indexReader, FIELD_CONTENT, cql3, null, null, null, true);
		assertEquals("Adjective preceded by Article", queryResult1.hits, (long) queryResult2.hits - queryResult3.hits);
	...
	public void basicSearchFullyAlignedWith1() throws IOException {
		...
		QueryResult queryResult4 = doQuery(indexReader, FIELD_CONTENT, cql4, null, null, null, false);
		QueryResult queryResult4disabled = doQuery(indexReader, FIELD_CONTENT, cql4, null, null, null, true);
		QueryResult queryResult5 = doQuery(indexReader, FIELD_CONTENT, cql5, null, null, null, false);
		QueryResult queryResult5disabled = doQuery(indexReader, FIELD_CONTENT, cql5, null, null, null, true);
		QueryResult queryResult6 = doQuery(indexReader, FIELD_CONTENT, cql6, null, null, null, false);
		QueryResult queryResult6disabled = doQuery(indexReader, FIELD_CONTENT, cql6, null, null, null, true);
	...
	public void basicSearchFullyAlignedWith2() throws IOException {
		...
		QueryResult queryResult1 = doQuery(indexReader, FIELD_CONTENT, cql, null, null, null, false);
		QueryResult queryResult1disabled = doQuery(indexReader, FIELD_CONTENT, cql, null, null, null, true);
		assertFalse("Fullyalignedwith: " + cql + " has no hits (" + queryResult1.hits + ")", queryResult1.hits == 0);
		assertEquals("Fullyalignedwith: - twoPhaseIterator", queryResult1.hits, queryResult1disabled.hits);
	}
	 ...
	 * @throws IOException Signals that an I/O exception has occurred.
	 */
	private static void createIndex(String configFile, HashMap<String, String> files) throws IOException {
		// analyzer
		...
		Analyzer mtasAnalyzer = CustomAnalyzer.builder(Paths.get("docker").toAbsolutePath())
				.addCharFilter("mtas", paramsCharFilterMtas).withTokenizer("mtas", paramsTokenizer).build();
	 ...
	 * @throws IOException Signals that an I/O exception has occurred.
	 */
	private static void addDoc(IndexWriter w, Integer id, String title, String file) throws IOException {
		try {
	 ...
	 */
	private MtasSpanQuery createQuery(String field, String cql, MtasSpanQuery ignore, Integer maximumIgnoreLength,
	 ...
	 * @throws IOException Signals that an I/O exception has occurred.
	 */
	private QueryResult doQuery(IndexReader indexReader, String field, String cql, MtasSpanQuery ignore,
			Integer maximumIgnoreLength, ArrayList<String> prefixes, boolean disableTwoPhaseIterator)
			throws IOException {
	 ...
	 * @throws IOException Signals that an I/O exception has occurred.
	 */
	private QueryResult doQuery(IndexReader indexReader, String field, MtasSpanQuery q, ArrayList<String> prefixes)
			throws IOException {
		QueryResult queryResult = new QueryResult();
		ListIterator<LeafReaderContext> iterator = indexReader.leaves().listIterator();
		IndexSearcher searcher = new IndexSearcher(indexReader);
		final float boost = 0;
		SpanWeight spanweight = q.rewrite(indexReader).createWeight(searcher, ScoreMode.COMPLETE_NO_SCORES, boost);
							...
							if (prefixes != null && !prefixes.isEmpty()) {
								...
								for (MtasTreeHit<String> term : terms) {
									queryResult.resultList.add(new QueryHit(lrc.docBase + spans.docID(),
											term.startPosition, term.endPosition, CodecUtil.termPrefix(term.data),
	 ...
	 * @throws IOException Signals that an I/O exception has occurred.
	 */
	private void testNumberOfHits(IndexReader indexReader, String field, List<String> cqls1, List<String> cqls2)
			throws IOException {
		int sum1 = 0;
		int sum2 = 0;
		int sum3 = 0;
		...
		assertEquals("twoPhaseIterator enabled for " + cqls1 + " (" + sum1 + ") and " + cqls2 + " (" + sum3 + ")", sum1,
				sum3);
		assertEquals("twoPhaseIterator disabled for " + cqls1 + " (" + sum2 + ") and " + cqls2 + " (" + sum4 + ")",
				sum2, sum4);
		assertEquals("twoPhaseIterator for " + cqls1 + " (" + sum1 + " / " + sum2 + ")", sum1, sum2);
	}
	...
	private static class QueryHit {
		 ...
		 */
		protected QueryHit(int docId, int startPosition, int endPosition, String prefix, String value) {
		}
	}
