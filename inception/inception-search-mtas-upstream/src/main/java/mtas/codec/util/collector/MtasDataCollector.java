    ...
    implements Serializable
{
    ...
    protected MtasDataCollector(String collectorType, String dataType, String statsType,
            SortedSet<String> statsItems, String sortType, String sortDirection, Integer start,
            Integer number, String segmentRegistration, String boundary)
        throws IOException
    {
        // set properties
        this.closed = false;
            ...
            if (segmentRegistration.equals(SEGMENT_BOUNDARY_ASC)
                ...
                if (boundary != null) {
                    ...
                    throw new IOException(
                            "did expect boundary with segmentRegistration " + segmentRegistration);
                }
            }
            else if (boundary != null) {
                throw new IOException(
                        "didn't expect boundary with segmentRegistration " + segmentRegistration);
            }
        }
    ...
    protected MtasDataCollector(String collectorType, String dataType, String statsType,
            SortedSet<String> statsItems, String sortType, String sortDirection, Integer start,
            Integer number, String[] subCollectorTypes, String[] subDataTypes,
    ...
    {
        // initialize
        this(collectorType, dataType, statsType, statsItems, sortType, sortDirection, start, number,
        ...
        if (subCollectorTypes != null) {
            ...
            if (subCollectorTypes.length > 1) {
                ...
                newSubDataTypes = Arrays.copyOfRange(subDataTypes, 1, subStatsTypes.length);
                newSubStatsTypes = Arrays.copyOfRange(subStatsTypes, 1, subStatsTypes.length);
                newSubStatsItems = Arrays.copyOfRange(subStatsItems, 1, subStatsItems.length);
                newSubSortTypes = Arrays.copyOfRange(subSortTypes, 1, subSortTypes.length);
     ...
     *             Signals that an I/O exception has occurred.
     */
    public void initNewList(int maxNumberOfTerms, String segmentName, int segmentNumber,
            String boundary)
        throws IOException
    {
        if (closed) {
    ...
    protected void increaseNewListSize() throws IOException
    {
        if (!closed) {
            ...
            if (hasSub) {
                ...
                newSubCollectorListNextLevel = new MtasDataCollector[newSize];
                System.arraycopy(tmpNewSubCollectorListNextLevel, 0, newSubCollectorListNextLevel,
                        0, tmpNewSize);
            }
        }
        else {
        ...
        throws IOException
    {
        if (!closed) {
            if (!collectorType.equals(DataCollector.COLLECTOR_TYPE_DATA)) {
                throw new IOException("collector should be " + DataCollector.COLLECTOR_TYPE_DATA);
            }
            else {
                if (newPosition > 0) {
                    ...
                    if (hasSub) {
                        newSubCollectorNextLevel = DataCollector.getCollector(subCollectorTypes[0],
                                subDataTypes[0], subStatsTypes[0], subStatsItems[0],
                                subSortTypes[0], subSortDirections[0], subStart[0], subNumber[0],
     ...
     *             Signals that an I/O exception has occurred.
     */
    protected final MtasDataCollector getSubCollector(String key, boolean increaseSourceNumber)
        throws IOException
    {
        if (!closed) {
            if (collectorType.equals(DataCollector.COLLECTOR_TYPE_DATA)) {
                throw new IOException("collector should be " + DataCollector.COLLECTOR_TYPE_LIST);
            }
            else if (key == null) {
                ...
                if ((newPosition > 0) && newKeyList[(newPosition - 1)].compareTo(key) >= 0) {
                    int i = newPosition;
                    do {
                        i--;
                ...
                // ready, only handle sub
                if (hasSub) {
                    newSubCollectorListNextLevel[newCurrentPosition] = DataCollector.getCollector(
                            subCollectorTypes[0], subDataTypes[0], subStatsTypes[0],
                            subStatsItems[0], subSortTypes[0], subSortDirections[0], subStart[0],
                            subNumber[0], newSubCollectorTypes, newSubDataTypes, newSubStatsTypes,
                            newSubStatsItems, newSubSortTypes, newSubSortDirections, newSubStart,
     ...
     * @param key
     *            the key
     * @param otherSubCollector
     *            the other sub collector
     * @return the mtas data collector
     */
    protected final MtasDataCollector<?, ?> addNewFrom(String key,
            MtasDataCollector<?, ?> otherSubCollector)
    {
        hasSub = true;
        ...
        this.subStatsItems = (SortedSet<String>[]) new SortedSet<?>[] {
                otherSubCollector.statsItems };
        ...
        try {
            newSubCollectorListNextLevel[newCurrentPosition] = DataCollector.getCollector(
                    subCollectorTypes[0], subDataTypes[0], subStatsTypes[0], subStatsItems[0],
                    subSortTypes[0], subSortDirections[0], subStart[0], subNumber[0],
                    newSubCollectorTypes, newSubDataTypes, newSubStatsTypes, newSubStatsItems,
                    newSubSortTypes, newSubSortDirections, newSubStart, newSubNumber,
     ...
     *             Signals that an I/O exception has occurred.
     */
    protected abstract boolean compareWithBoundary(T1 value, T1 boundary) throws IOException;
     ...
     *             Signals that an I/O exception has occurred.
     */
    protected abstract T1 lastForComputingSegment(T1 value, T1 boundary) throws IOException;
     ...
     *             Signals that an I/O exception has occurred.
     */
    protected abstract T1 boundaryForSegment(String segmentName) throws IOException;
     ...
     *             Signals that an I/O exception has occurred.
     */
    protected abstract T1 boundaryForSegmentComputing(String segmentName) throws IOException;
            ...
            if (segmentRegistration != null) {
                ...
                for (Entry<String, T1> entry : keyValueList.entrySet()) {
                    if (tmpSegmentValueBoundary == null
                            || compareWithBoundary(entry.getValue(), tmpSegmentValueBoundary)) {
        ...
        if (!closed && segmentRegistration != null) {
            if (segmentRegistration.equals(SEGMENT_SORT_ASC)
                ...
                if (segmentRegistration.equals(SEGMENT_SORT_ASC)
                    ...
                    for (Entry<String, Map<String, T1>> entry : segmentKeyValueList.entrySet()) {
                    ...
                    for (Entry<String, Map<String, T1>> entry : segmentKeyValueList.entrySet()) {
                        ...
                        for (Entry<String, T1> subEntry : keyValueList.entrySet()) {
                            if (tmpSegmentValueBoundaryForComputing == null || compareWithBoundary(
                                    subEntry.getValue(), tmpSegmentValueBoundaryForComputing)) {
                ...
                throw new IOException("not for segmentRegistration " + segmentRegistration);
            }
        }
        else {
            throw new IOException(
                    "already closed or no segmentRegistration (" + segmentRegistration + ")");
        }
    }
     ...
     *             Signals that an I/O exception has occurred.
     */
    public String validateSegmentValue(T1 value, int maximumNumber, int segmentNumber)
        throws IOException
    {
        if (!closed) {
            if (segmentRegistration != null) {
     ...
     *             Signals that an I/O exception has occurred.
     */
    public String validateSegmentValue(String key, T1 value, int maximumNumber, int segmentNumber,
            boolean test)
        throws IOException
    {
        if (!closed) {
            if (segmentRegistration != null) {
                if (maximumNumber > 0) {
                    T1 tmpSegmentValueMaxListMin = segmentValueTopListLast.get(segmentName);
                    T1 tmpSegmentValueBoundary = segmentValuesBoundary.get(segmentName);
                    if (segmentValueTopList.size() < maximumNumber) {
                        if (!test) {
                            ...
                            if (segmentValueTopList.size() == maximumNumber) {
                                tmpSegmentValueMaxListMin = segmentValueTopListLast
                                        .get(segmentName);
                                segmentValueTopListLast.put(segmentName, tmpSegmentValueMaxListMin);
                                segmentValuesBoundary.put(segmentName,
            ...
            if (currentExisting) {
                ...
                for (Entry<String, Integer> entry : errorListItem.entrySet()) {
                    if (item.containsKey(entry.getKey())) {
                        item.put(entry.getKey(), item.get(entry.getKey()) + entry.getValue());
                    }
                    else {
     ...
     *             Signals that an I/O exception has occurred.
     */
    private boolean sortedAndUnique(String[] keyList, int size) throws IOException
    {
        if (!closed) {
     ...
     *             Signals that an I/O exception has occurred.
     */
    private int[][] computeSortAndUniqueMapping(String[] keyList, int size) throws IOException
    {
        if (!closed) {
            if (size > 0) {
            ...
            for (int i = 0; i < mapping.length; i++) {
                ...
                for (int j = 0; j < mapping[i].length; j++) {
                    if (j == 0) {
                        ...
                        newErrorNumber[i] += errorNumber[mapping[i][j]];
                        for (Entry<String, Integer> entry : errorList[mapping[i][j]].entrySet()) {
     ...
     *             Signals that an I/O exception has occurred.
     */
    public abstract MtasDataCollector add(long valueSum, long valueN) throws IOException;
     ...
     *             Signals that an I/O exception has occurred.
     */
    public abstract MtasDataCollector add(long[] values, int number) throws IOException;
     ...
     *             Signals that an I/O exception has occurred.
     */
    public abstract MtasDataCollector add(double valueSum, long valueN) throws IOException;
     ...
     *             Signals that an I/O exception has occurred.
     */
    public abstract MtasDataCollector add(double[] values, int number) throws IOException;
     ...
     *             Signals that an I/O exception has occurred.
     */
    public abstract MtasDataCollector add(String key, long[] values, int number) throws IOException;
     ...
     *             Signals that an I/O exception has occurred.
     */
    public abstract MtasDataCollector add(String key, double valueSum, long valueN)
        throws IOException;
    ...
    public void close() throws IOException
    {
        if (!closed) {
            ...
            if (collectorType.equals(DataCollector.COLLECTOR_TYPE_LIST)) {
                ...
                result = new MtasDataCollectorResult<>(collectorType, sortType, sortDirection,
                        basicList, start, number);
                // reduce
                if (segmentRegistration != null) {
                    if (segmentRegistration.equals(SEGMENT_SORT_ASC)
                            || segmentRegistration.equals(SEGMENT_SORT_DESC)) {
