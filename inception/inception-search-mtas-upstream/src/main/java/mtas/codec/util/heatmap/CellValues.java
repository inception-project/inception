...
public class CellValues
{
     ...
     * @param values
     *            the values
     * @param functionValuesLong
     *            the function values long
     * @param functionValuesDouble
     *            the function values double
     * @param functionValuesError
     *            the function values error
     */
    ...
    public void merge(CellValues newCellValues)
    {
        ...
        System.arraycopy(newCellValues.values, 0, newValues, values.length,
                newCellValues.values.length);
        // merge function values long
        for (int i = 0; i < functionValuesLong.length; i++) {
            newFunctionValuesLong[i] = new long[functionValuesLong[i].length
                    + newCellValues.functionValuesLong[i].length];
            System.arraycopy(functionValuesLong[i], 0, newFunctionValuesLong[i], 0,
                    functionValuesLong[i].length);
            System.arraycopy(newCellValues.functionValuesLong[i], 0, newFunctionValuesLong[i],
                    functionValuesLong[i].length, newCellValues.functionValuesLong[i].length);
        }
        ...
        for (int i = 0; i < functionValuesDouble.length; i++) {
            ...
            System.arraycopy(functionValuesDouble[i], 0, newFunctionValuesDouble[i], 0,
                    functionValuesDouble[i].length);
        ...
        for (int i = 0; i < functionValuesError.length; i++) {
            for (Entry<String, Integer> entry : newCellValues.functionValuesError[i].entrySet()) {
                if (functionValuesError[i].containsKey(entry.getKey())) {
                    functionValuesError[i].put(entry.getKey(),
                            functionValuesError[i].get(entry.getKey()) + entry.getValue());
                }
                else {
