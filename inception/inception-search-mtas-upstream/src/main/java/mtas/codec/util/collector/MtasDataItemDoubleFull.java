    ...
    extends MtasDataItemFull<Double, Double>
{
    ...
    {
        super(Arrays.stream(value).boxed().toArray(Double[]::new), sub, statsItems, sortType,
                sortDirection, errorNumber, errorList, new MtasDataDoubleOperations(),
                sourceNumber);
    }
    ...
    protected HashMap<String, Object> getDistribution(String argument)
    {
        ...
        int tmpD = Double.valueOf(Math.max(0, 1 + Math.ceil(-1 * Math.log10(step)))).intValue();
        ...
        for (Double v : fullValues) {
            if (v >= start && v <= end) {
                int i = Math.min(Double.valueOf(Math.floor((v - start) / step)).intValue(),
                        (number - 1));
                list[i]++;
            }
        }
    ...
    public MtasDataItemNumberComparator<Double> getCompareValue1()
    {
        ...
        case CodecUtil.STATS_TYPE_SUMSQ:
            return new MtasDataItemNumberComparator<>(stats.getSumsq(), sortDirection);
        default:
            return null;
        }
    }
