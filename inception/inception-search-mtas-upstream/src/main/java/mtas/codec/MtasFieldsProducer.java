 ...
 */
public class MtasFieldsProducer
    extends FieldsProducer
{
     ...
     */
    public MtasFieldsProducer(SegmentReadState state, String name) throws IOException
    {
        ...
        addIndexInputToList("indexDocId",
                openMtasFile(state, name, MtasCodecPostingsFormat.MTAS_INDEX_DOC_ID_EXTENSION),
                postingsFormatName);
        addIndexInputToList("indexObjectId",
                openMtasFile(state, name, MtasCodecPostingsFormat.MTAS_INDEX_OBJECT_ID_EXTENSION),
                postingsFormatName);
        try {
            addIndexInputToList("doc", openMtasFile(state, name,
                    MtasCodecPostingsFormat.MTAS_DOC_EXTENSION, version, version),
                    postingsFormatName);
            addIndexInputToList("indexObjectPosition", openMtasFile(state, name,
                    MtasCodecPostingsFormat.MTAS_INDEX_OBJECT_POSITION_EXTENSION, version, version),
                    postingsFormatName);
            addIndexInputToList("indexObjectParent", openMtasFile(state, name,
                    MtasCodecPostingsFormat.MTAS_INDEX_OBJECT_PARENT_EXTENSION, version, version),
                    postingsFormatName);
        }
        catch (IndexFormatTooOldException e) {
            log.debug("Error", e);
            throw new IOException("This MTAS doesn't support your index version, please upgrade");
        }
     ...
     *             Signals that an I/O exception has occurred.
     */
    private String addIndexInputToList(String name, IndexInput in, String postingsFormatName)
        throws IOException
    {
     ...
     *             Signals that an I/O exception has occurred.
     */
    private IndexInput openMtasFile(SegmentReadState state, String name, String extension,
        ...
        throws IOException
    {
        ...
        try {
            ...
            object.close();
            log.debug("Error", e);
            throw new IndexFormatTooOldException(e.getMessage(), e.getVersion(), e.getMinVersion(),
                    e.getMaxVersion());
        }
