 ...
 */
public class MtasSpanRecurrenceSpans
    extends MtasSpans
{
    ...
    {
        resetQueue();
        return (spans.advance(target) == NO_MORE_DOCS) ? NO_MORE_DOCS : toMatchDoc();
    }
            ...
            if (!queueSpans.isEmpty()) {
                ...
                for (int i = 0; i < queueSpans.size(); i++) {
                    ...
                    if (match.endPosition() == span.startPosition()
                        ...
                        findMatches(new Match(match.startPosition(), span.endPosition()), (n - 1));
    ...
    private List<Match> expandWithIgnoreItem(int docId, Match match)
    {
        ...
        try {
            Set<Integer> ignoreList = ignoreItem.getFullEndPositionList(docId, match.endPosition);
            if (ignoreList != null) {
