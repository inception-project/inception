    ...
    extends MtasSpanQuery
{
    ...
    public MtasSpanQuery rewrite(IndexSearcher searcher) throws IOException
    {
        ...
        if (!newClause.equals(clause)) {
            return new MtasSpanEndQuery(newClause).rewrite(searcher);
    ...
    public MtasSpanWeight createWeight(IndexSearcher searcher, ScoreMode scoreMode, float boost)
        throws IOException
    {
        SpanWeight spanWeight = ((SpanQuery) searcher.rewrite(clause)).createWeight(searcher,
                scoreMode, boost);
