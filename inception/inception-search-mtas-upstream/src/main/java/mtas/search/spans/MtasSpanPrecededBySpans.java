 ...
 */
public class MtasSpanPrecededBySpans
    extends MtasSpans
{
    ...
    public MtasSpanPrecededBySpans(MtasSpanPrecededByQuery query,
            MtasSpanPrecededByQuerySpans spans1, MtasSpanPrecededByQuerySpans spans2)
    {
        super();
        docId = -1;
        this.query = query;
    ...
    {
        if (calledNextStartPosition) {
            return noMorePositions ? 0 : spans1.spans.endPosition() - spans1.spans.startPosition();
        }
        else {
            return 0;
        }
    ...
    private boolean goToNextStartPosition() throws IOException
    {
        ...
        while ((nextSpans1StartPosition = spans1.spans.nextStartPosition()) != NO_MORE_POSITIONS) {
            if (nextSpans1StartPosition == lastSpans2EndPosition) {
                ...
                else if (previousSpans2EndPositions.contains(nextSpans1StartPosition)) {
                    return true;
                }
                // try to find match
