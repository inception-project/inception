    ...
    extends MtasSpans
{
        ...
        while ((nextSpans1StartPosition = spans1.spans.nextStartPosition()) != NO_MORE_POSITIONS) {
            if (spans1.spans.docID() == spans2.spans.docID()) {
                ...
                if (nextSpans1StartPosition == lastSpans2StartPosition) {
                    ...
                    // contained
                    if (collectAndCheckLastSpans(nextSpans1StartPosition, nextSpans1EndPosition)) {
                        return true;
                    }
                    else {
                        // continue
     ...
     *
     * @param nextSpans1StartPosition
     *            the next spans 1 start position
     * @param nextSpans1EndPosition
     *            the next spans 1 end position
     * @return true, if successful
     * @throws IOException
     *             Signals that an I/O exception has occurred.
     */
    private boolean collectAndCheckLastSpans(int nextSpans1StartPosition, int nextSpans1EndPosition)
        throws IOException
    {
        // check next
        if (nextSpans1StartPosition == nextSpans2StartPosition
