 ...
 *
 * <p>
 * To quickly execute range queries in Apache Lucene, a range is divided recursively into multiple
 * intervals for searching: The center of the range is searched only with the lowest possible
 * precision in the trie, while the boundaries are matched more exactly. This reduces the number of
 * terms dramatically.
 *
 * <p>
 ...
 *
 * <p>
 * For easy usage, the trie algorithm is implemented for indexing inside {@code
...
public final class LegacyNumericUtils
{
    ...
    /**
     * Integers are stored at lower precision by shifting off lower bits. The shift count is stored
     * as <code>SHIFT_START_INT+shift</code> in the first byte
     */
     ...
     */
    public static void longToPrefixCoded(final long val, final int shift,
            final BytesRefBuilder bytes)
    {
        // ensure shift is 0..63
        if ((shift & ~0x3f) != 0) {
            throw new IllegalArgumentException(
                    "Illegal shift value, must be 0..63; got shift=" + shift);
        }
    ...
    public static void intToPrefixCoded(final int val, final int shift, final BytesRefBuilder bytes)
    {
        ...
        if ((shift & ~0x1f) != 0) {
            throw new IllegalArgumentException(
                    "Illegal shift value, must be 0..31; got shift=" + shift);
        }
     ...
     *             if the supplied {@link BytesRef} is not correctly prefix encoded.
     */
    public static int getPrefixCodedLongShift(final BytesRef val)
    {
        final int shift = val.bytes[val.offset] - SHIFT_START_LONG;
        if (shift > 63 || shift < 0) {
            throw new NumberFormatException("Invalid shift value (" + shift
            ...
            if (b < 0) {
                throw new NumberFormatException(
                        "Invalid prefixCoded numerical value representation (byte "
                                + Integer.toHexString(b & 0xff) + " at position " + (i - val.offset)
                                + " is invalid)");
            }
            sortableBits |= b;
        }
    ...
    /**
     * Splits a long range recursively. You may implement a builder that adds clauses to a
     * {@link org.apache.lucene.search.BooleanQuery} for each call to its
     * {@link LongRangeBuilder#addRange(BytesRef,BytesRef)} method.
     *
     * <p>
     * This method is used by {@code org.apache.solr.legacy.LegacyNumericRangeQuery}.
     */
    ...
    /**
     * Splits an int range recursively. You may implement a builder that adds clauses to a
     * {@link org.apache.lucene.search.BooleanQuery} for each call to its
    ...
    private static void splitRange(final Object builder, final int valSize, final int precisionStep,
    ...
    private static void addRange(final Object builder, final int valSize, long minBound,
            long maxBound, final int shift)
    {
    ...
    public abstract static class LongRangeBuilder
    {
        ...
        /**
         * Overwrite this method, if you like to receive the raw long range bounds. You can use this
         * for e.g. debugging purposes (print out range bounds).
         */
        public void addRange(final long min, final long max, final int shift)
        {
            final BytesRefBuilder minBytes = new BytesRefBuilder(),
                    maxBytes = new BytesRefBuilder();
    ...
    public abstract static class IntRangeBuilder
    {
        ...
        /**
         * Overwrite this method, if you like to receive the raw int range bounds. You can use this
         * for e.g. debugging purposes (print out range bounds).
         */
    ...
    public static TermsEnum filterPrefixCodedLongs(TermsEnum termsEnum)
    {
        return new SeekingNumericFilteredTermsEnum(termsEnum)
        {
            ...
            {
                return LegacyNumericUtils.getPrefixCodedLongShift(term) == 0 ? AcceptStatus.YES
                        : AcceptStatus.END;
            }
        };
    }
    ...
    public static TermsEnum filterPrefixCodedInts(TermsEnum termsEnum)
    {
        return new SeekingNumericFilteredTermsEnum(termsEnum)
        {
            ...
            protected AcceptStatus accept(BytesRef term)
            {
                return LegacyNumericUtils.getPrefixCodedIntShift(term) == 0 ? AcceptStatus.YES
                        : AcceptStatus.END;
            }
        };
    }
     ...
     */
    private abstract static class SeekingNumericFilteredTermsEnum
