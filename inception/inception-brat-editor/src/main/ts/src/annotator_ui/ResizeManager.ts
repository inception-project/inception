...
export class ResizeManager {
    ...
    private handleResizeHandleReleased(e: Event): void {
        if (!(e instanceof CustomEvent) || !(e.detail?.event instanceof MouseEvent) || !this.id)
            return;
    ...
    private handleMouseOver(e: Event): void {
        ...
        if (
            this.beginHandle?.getHandle().contains(e.target) ||
            this.endHandle?.getHandle().contains(e.target)
        ) {
            this.cancelAutoHide();
            return;
        }
    ...
    autoHide(): void {
        function hideOrRescheduleAutoHide(context: ResizeManager) {
            if (context.beginHandle?.isDragging() || context.endHandle?.isDragging()) {
                context.hideTimer = window.setTimeout(
                    () => hideOrRescheduleAutoHide(context),
                    1000
                );
            } else {
                context.hide();
