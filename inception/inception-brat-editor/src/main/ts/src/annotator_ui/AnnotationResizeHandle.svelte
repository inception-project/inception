...
<script lang="ts">
    ...
    run(() => {
        ...
        if (highlight && scrollContainer && rects.length > 0) {
            ...
            style.borderWidth
                .split(' ')
                .forEach((r) => (borderWidth = Math.max(borderWidth, parseInt(r))));
            ...
            if (!dragging) {
                ...
                style.borderRadius
                    .split(' ')
                    .forEach((r) => (borderRadius = Math.max(borderRadius, parseInt(r))));
            } else {
                borderRadius = 0;
            }
        }
    ...
    function handleDrag(event: MouseEvent) {
        ...
        const isRectConsistent =
            containerRect.left <= rect.x &&
            rect.x <= containerRect.right &&
            containerRect.top <= rect.y &&
...
<span
    bind:this={marker}
    class="marker"
    style:visibility={markerVisibility}
...
<style lang="scss">
    .handle {
        ...
        .inner-handle {
            pointer-events: none;
            display: inline-flex;
            border: var(--handle-width) rgba(0, 0, 0, 0);
