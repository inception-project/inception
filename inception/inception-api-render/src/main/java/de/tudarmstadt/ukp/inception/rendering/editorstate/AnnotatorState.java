...
/**
 * Covers information about the state of the annotation editor component that is
 * relevant across
 * cycles.
 */
public interface AnnotatorState
    ...
    /**
     * @param aTimeStamp
     *                   the timestamp when the annotation document was last changed
     *                   on this. This value
     *                   must be set explicitly whenever the annotation document is
     *                   loaded by the editor.
     *                   It can be used to detect modifications to the file on disk
     *                   which might make it
     *                   incompatible with the current state of the annotation
     *                   editor (in particular it
    ...
    // Annotation behavior
    //
    // Control which kinds of annotations are created when an annotation creation
    // action is
    // triggered and also what happens after the annotation has been created (e.g.
    // auto-forward)
    // ---------------------------------------------------------------------------------------------

    // REC: would be very nice if we didn't need the mode - the behaviors specific
    // to annotation,
    // curation, automation, correction, etc. should be local to the respective
    // modules / pages
    @Deprecated
    Mode getMode();
    ...
    //
    // These are optionally used when a new annotation is created to pre-fill
    // feature values using
    // those of the last annotation of the same type. This can be useful when many
    // annotations of
     ...
     * @return if user is viewing other people's work (read-only), but not as
     *         Curation User
     */
    ...
    // ---------------------------------------------------------------------------------------------
    // Constraints
    // REC: we cache the constraints when a document is opened because parsing them
    // takes some time
    // ---------------------------------------------------------------------------------------------
