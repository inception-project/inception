    ...
    extends GenericPanel<Project>
{
    ...
    public LayerTemplateSelectionDialogPanel(String aId, IModel<Project> aProjectModel,
            IModel<List<LayerInitializer>> aInitializers)
    {
        super(aId, aProjectModel);

        var initializers = new ListView<LayerInitializer>("templates", aInitializers)
        {
            private static final long serialVersionUID = 1L;

            @Override
            protected void populateItem(ListItem<LayerInitializer> aItem)
            {
                ...
                aItem.queue(new Image("thumbnail", aItem.getModel()
                        .map(LayerInitializer::getThumbnail).map($ -> $.orElse(NO_THUMBNAIL))));
            }
        };
        queue(initializers);
    ...
    private void actionCreateLayer(AjaxRequestTarget aTarget, LayerInitializer aInitializer)
    {
        send(this, Broadcast.BUBBLE, new LayerTemplateSelectedEvent(aTarget, aInitializer));
