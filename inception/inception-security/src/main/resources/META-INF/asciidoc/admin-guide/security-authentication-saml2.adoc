...
Certain SAML IdPs like Microsoft Azure AD (Entra ID) may only offer HTTP POST binding for SAML responses.
To allow these IdPs to work with {product-name}, you can add the following lines to your `settings.properties`:
...
Alternatively, consider using OAuth/OIDC (which does not require these settings to be made) or consider using a
different IdP like e.g. Keycloak which supports HTTP Redirect binding for the SAML response.
