...
NOTE: When group-to-role mapping is disabled (the default), roles assigned manually by an
      administrator are preserved across logins. Enabling group-to-role mapping means that roles
      are re-derived from the OIDC token on every login, so any manually assigned roles will be
      overwritten.
...
| Group name (or path) that grants `ROLE_ADMIN`.
  Note that `ROLE_ADMIN` does **not** imply `ROLE_USER`.
  An admin who also needs to use the application as a regular user must be placed in both the admin group and the user group.
