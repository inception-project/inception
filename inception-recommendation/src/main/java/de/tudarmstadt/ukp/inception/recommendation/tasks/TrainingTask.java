    ...
    extends Task
{
    ...
    public void run()
    {
        ...
        List<EvaluatedRecommender> recommenders=new ArrayList<>();
        ...
        for (AnnotationLayer layer : annoService.listAnnotationLayer(project)) {
            ...
            List<EvaluatedRecommender> subRecommenders = recommendationService
                    .getActiveRecommenders(user, layer);
    
            if (subRecommenders.isEmpty()) {
                log.debug("[{}][{}][{}]: No active recommenders, skipping training.",
                        getId(), user.getUsername(), layer.getUiName());
                continue;
            }
            recommenders.addAll(subRecommenders);
        }
