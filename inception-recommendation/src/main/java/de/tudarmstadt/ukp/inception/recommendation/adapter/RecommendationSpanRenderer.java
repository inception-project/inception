    ...
    implements RecommendationTypeRenderer
{
            ...
            AnnotationLayer layer)
    {
        ...
        for (List<AnnotationObject> token: recommendations) {
            Map<String, Map<Integer, AnnotationObject>> labelMap = new HashMap<>();
            ...
            for (AnnotationObject ao: token) {          
                if (ao.getAnnotation() != null) {       
                    ...
                    if (!labelMap.containsKey(ao.getAnnotation())
                        ...
                        Map<Integer, AnnotationObject> confidencePerClassifier;
            ...
            for (String label : labelMap.keySet()) {
                ...
                for (Entry<Integer, AnnotationObject> classifier : labelMap.get(label).entrySet()) {
            ...
            for (String label : labelMap.keySet()) {
                ...
                Map<Integer, AnnotationObject> confidencePerClassifier = labelMap.get(label);
                
                boolean first = true;
                for (Integer recommenderId: confidencePerClassifier.keySet()) {
                    AnnotationObject ao = confidencePerClassifier.get(recommenderId);
                    VID vid = new VID(RecommendationEditorExtension.BEAN_NAME, (int) layer.getId(),
                            ao.getRecommenderId(), ao.getId(), VID.NONE, VID.NONE);
