    ...
    public void run()
    {
        ...
        nextDocument: for (SourceDocument document : documents) {
            ...
            if (!originalCas.isPresent()) {
                try {
                    originalCas = Optional.of(documentService
                            .readAnnotationCas(document, user.getUsername()).getCas());
                }
                catch (IOException e) {
                    log.error(
                ...
                try {
                    ...
                    log.error(
                            "Cannot clone annotation CAS for user [{}] of document "
                                    + "[{}]({}) in project [{}]({}) - skipping document",
                            user.getUsername(), document.getName(), document.getId(),
                            project.getName(), project.getId(), e);
                    continue nextDocument;
            ...
            model = PredictionUtil.getPredictions(document, annoService, recommendationService,
                    originalCas, predictionCas, user, project, learningRecordService);
        }
