...
//    }
    ...
    public void onPrecitionsSwitched(PredictionsSwitchedEvent aEvent)
    {
        if (annotatorState != null && selectedLayer != null) {
            ...
            listOfRecommendationsForEachToken.removeIf(recommendationList -> recommendationList.isEmpty());
            // remove duplicate recommendations
            listOfRecommendationsForEachToken = listOfRecommendationsForEachToken.stream().map(it -> removeDuplicateRecommendations(it)).collect(Collectors.toList());
        }
    }
    ...
    @Override
    public void setAnnotatorStateAndLayer(AnnotatorState aAnnotatorState, AnnotationLayer
        aSelectedLayer)
    {
        annotatorState = aAnnotatorState;
        selectedLayer = aSelectedLayer;
    }
}
